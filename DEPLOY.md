...
## قسم التوظيف لا يظهر في لوحة الإدارة (أونلاين)

السايدبار يعتمد على صلاحية `view-jobs`. محلياً تُضاف عبر `db:seed`؛ على السيرفر شغّل:

```bash
php artisan migrate
# أو إذا كانت الـ migrations مُنفّذة مسبقاً:
php artisan permissions:sync-jobs
php artisan permission:cache-reset
```

ثم **سجّل خروجاً وادخل** من جديد.

الأدوار التي ترى «التوظيف»: `super_admin`، `admin`، `hr`.
