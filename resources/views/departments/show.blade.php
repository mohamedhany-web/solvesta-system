        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 truncate">{{ $department->name }}</h1>
                    <span class="inline-flex items-center px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs sm:text-sm font-medium flex-shrink-0 {{ $department->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $department->is_active ? 'نشط' : 'غير نشط' }}
                    </span>
                </div>
                <p class="text-sm sm:text-base text-gray-600">{{ $department->code }}</p>
            </div>
            <a href="{{ route('departments.edit', $department) }}" class="bg-blue-600 text-white px-5 py-2.5 sm:px-6 sm:py-3 rounded-lg hover:bg-blue-700 transition-all duration-200 flex items-center justify-center shadow-sm text-sm sm:text-base">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $stats['total_employees'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $stats['active_projects'] }}</p>
                    <p class="text-xs text-gray-500 mt-1">من {{ $stats['active_projects'] + $stats['completed_projects'] }} مشروع</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-gray-900">{{ number_format($stats['total_budget'], 0) }} ج.م</p>
                </div>
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 lg:p-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900">{{ number_format($stats['average_salary'], 0) }} ج.م</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-4 sm:px-5 lg:px-6 py-3 sm:py-4 border-b border-gray-200 flex items-center justify-between">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-900">الموظفون ({{ $department->employees->count() }})</h3>
                </div>
                ...
                <div class="divide-y divide-gray-200">
                    ...
                    <div class="p-4 hover:bg-gray-50 transition-colors duration-200">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="text-left">
                                <p class="text-sm font-medium text-gray-900">{{ number_format($employee->salary, 0) }} ج.م</p>
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $employee->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-4 sm:px-5 lg:px-6 py-3 sm:py-4 border-b border-gray-200 flex items-center justify-between">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-900">المشاريع ({{ $department->projects->count() }})</h3>
                </div>
                        ...
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-1">
                                    ...
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-{{ $project->status_color }}-100 text-{{ $project->status_color }}-800">
                                        {{ $project->status }}
                                    </span>
                                </div>
                                ...
                                <div class="flex items-center gap-4 text-sm text-gray-500">
                                    <span class="flex items-center">
                                        ...
                                        {{ $project->client ? $project->client->name : 'لا يوجد عميل' }}
                                    </span>
                                    <span class="flex items-center">
                                        <svg class="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                            ...
                            <div class="text-left mr-4">
                                <p class="text-lg font-bold text-gray-900">{{ number_format($project->budget, 0) }} ج.م</p>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 lg:p-6">
                ...
                <div class="text-center">
                    ...
                    <h4 class="font-bold text-gray-900 mb-1">{{ $department->manager->first_name }} {{ $department->manager->last_name }}</h4>
                    <p class="text-sm text-gray-600 mb-3">{{ $department->manager->position }}</p>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 lg:p-6">
                ...
                <div class="space-y-2">
                    <a href="{{ route('departments.edit', $department) }}" class="flex items-center p-3 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-lg transition-colors duration-200">
                    ...
                    <a href="{{ route('employees.index') }}?department={{ $department->id }}" class="flex items-center p-3 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-lg transition-colors duration-200">
