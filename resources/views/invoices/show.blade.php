    ...
    <div class="bg-white overflow-hidden print:shadow-none print:border-0 print:bg-white" style="max-width: 210mm; margin: 0 auto; box-shadow: 0 0 0 1px rgba(0,0,0,0.1);">
        ...
        <div class="bg-gray-900 text-white py-6 px-10">
            <div class="flex justify-between items-start">
                ...
                <div class="flex items-start gap-5">
                    ...
                    @if($logoExists)
                        <div class="flex-shrink-0 bg-white p-2 rounded">
                            <img src="{{ asset('storage/' . $logoPath) }}" alt="Logo" class="h-16 w-auto object-contain">
                        </div>
                    @else
                    ...
                    @endif
                    <div class="flex-1">
                        <h2 class="text-xl font-bold mb-2">{{ $companyName }}</h2>
                ...
                <div class="text-left bg-white text-gray-900 px-6 py-4 rounded-lg border-2 border-gray-700">
                    ...
                    <div class="text-3xl font-bold">{{ number_format($invoice->total_amount, 2) }}</div>
                ...
                <div>
                    <div class="bg-white border-r-4 border-gray-900 p-4 rounded">
                        ...
                        <div class="space-y-1.5">
                            <h4 class="font-bold text-gray-900 text-sm">{{ \App\Helpers\SettingsHelper::getCompanyName() }}</h4>
                            <p class="text-xs text-gray-600 hide-address-print">{{ \App\Helpers\SettingsHelper::getCompanyAddress() }}</p>
                            <p class="text-xs text-gray-700">
                                <span class="font-semibold">الهاتف:</span> {{ \App\Helpers\SettingsHelper::getCompanyPhone() }}
                            </p>
                            <p class="text-xs text-gray-700">
                                <span class="font-semibold">البريد:</span> {{ \App\Helpers\SettingsHelper::getCompanyEmail() }}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white border-r-4 border-gray-900 p-4 rounded">
                        ...
                        <div class="space-y-2 text-xs">
                            ...
                            <div class="flex justify-between items-center py-1 border-b border-gray-100">
                                ...
                                <span class="font-semibold text-gray-900">
                                    @if($invoice->due_date)
                                        {{ $invoice->due_date instanceof \Carbon\Carbon ? $invoice->due_date->format('Y/m/d') : \Carbon\Carbon::parse($invoice->due_date)->format('Y/m/d') }}
                                    @else
                                        -
                                    @endif
                                </span>
        ...
        <div class="px-10 py-3 bg-blue-50 border-t border-gray-200">
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs text-gray-900 font-medium">{{ $invoice->project->name }}</span>
                @if($invoice->project->description)
                <span class="text-xs text-gray-600">- {{ $invoice->project->description }}</span>
                @endif
            </div>
        </div>
        @endif
        ...
        <div class="px-10 py-3 bg-purple-50 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs text-gray-900 font-medium">{{ $invoice->contract->title }}</span>
                    <span class="text-xs text-gray-600">({{ $invoice->contract->contract_number }})</span>
                </div>
                <a href="{{ route('contracts.show', $invoice->contract) }}" class="text-xs text-purple-600 hover:text-purple-800 font-medium">
                    عرض العقد →
                </a>
            </div>
        </div>
                    ...
                    <table class="w-full bg-white border border-gray-300 rounded-lg overflow-hidden">
                        <tbody>
                            <tr class="border-b border-gray-200">
                                ...
                                <td class="py-3 px-5 text-left text-sm font-bold text-gray-900">{{ number_format($invoice->subtotal ?? $invoice->amount, 2) }} ج.م</td>
                            </tr>
                            @if($invoice->tax_amount > 0)
                            <tr class="border-b border-gray-200">
                                <td class="py-3 px-5 text-right text-sm text-gray-700 font-semibold">الضريبة ({{ $invoice->tax_rate ?? 0 }}%):</td>
                                <td class="py-3 px-5 text-left text-sm font-bold text-gray-900">{{ number_format($invoice->tax_amount, 2) }} ج.م</td>
                            </tr>
                            @endif
                            @if($invoice->discount_amount > 0)
                            <tr class="border-b border-gray-200">
                                <td class="py-3 px-5 text-right text-sm text-gray-700 font-semibold">الخصم:</td>
                                <td class="py-3 px-5 text-left text-sm font-bold text-gray-900">- {{ number_format($invoice->discount_amount, 2) }} ج.م</td>
                            </tr>
                            @endif
                            <tr class="bg-gray-900 text-white">
                                <td class="py-4 px-5 text-right text-base font-bold">المجموع الكلي:</td>
                                <td class="py-4 px-5 text-left text-xl font-bold">{{ number_format($invoice->total_amount, 2) }} ج.م</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="px-10 py-6 border-t-2 border-gray-300">
            ...
            <div class="mb-5">
                ...
                <div class="bg-gray-50 border-r-4 border-gray-700 p-4 rounded">
                    @if($invoice->notes)
                        <p class="text-sm text-gray-700 leading-relaxed mb-2">{{ $invoice->notes }}</p>
                    @endif
                    @if($financialNotes)
                        <p class="text-sm text-gray-700 leading-relaxed">{{ $financialNotes }}</p>
                    @endif
                </div>
            </div>
            @endif
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="bg-white border border-gray-200 p-4 rounded">
                        ...
                        <div class="space-y-2 text-xs text-gray-700">
                            ...
                            @endphp
                            <p><span class="font-semibold text-gray-900">طريقة الدفع:</span> {{ $paymentMethods ?: 'تحويل بنكي أو شيك' }}</p>
                            <p><span class="font-semibold text-gray-900">مدة السداد:</span> {{ $days }} يوم من تاريخ الإصدار</p>
                        </div>
                    </div>
                    <div class="bg-white border border-gray-200 p-4 rounded">
                        ...
                        <div class="space-y-2 text-xs text-gray-700">
                            ...
                            @endphp
                            @if($bankName)
                                <p><span class="font-semibold text-gray-900">البنك:</span> {{ $bankName }}</p>
                            @endif
                            @if($accountNumber)
                                <p><span class="font-semibold text-gray-900">رقم الحساب:</span> {{ $accountNumber }}</p>
                            @endif
                            @if($swiftCode)
                                <p><span class="font-semibold text-gray-900">Swift Code:</span> {{ $swiftCode }}</p>
                            @endif
                            @if($iban)
                                <p><span class="font-semibold text-gray-900">IBAN:</span> {{ $iban }}</p>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-6 pt-5 border-t-2 border-gray-300 text-center">
                <div class="bg-gray-900 text-white py-4 px-6 rounded-lg">
                    ...
                    <div class="flex justify-center items-center gap-4 text-xs text-gray-300">
                        <span>للاستفسارات: {{ \App\Helpers\SettingsHelper::getCompanyEmail() }}</span>
                        <span>|</span>
                        <span>{{ \App\Helpers\SettingsHelper::getCompanyPhone() }}</span>
                    </div>
                    <p class="text-xs text-gray-400 mt-3">© {{ date('Y') }} {{ \App\Helpers\SettingsHelper::getCompanyName() }}. جميع الحقوق محفوظة.</p>
                </div>
            </div>
        </div>
    </div>
