...
<div class="w-full max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('meetings.show', $meeting) }}" class="bg-gray-600 text-white px-6 py-3 rounded-xl hover:bg-gray-700 transition-all duration-200 flex items-center shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <form action="{{ route('meetings.update', $meeting) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('description') border-red-500 @enderror">{{ old('description', $meeting->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="internal" {{ old('type', $meeting->type) == 'internal' ? 'selected' : '' }}>داخلي</option>
                        <option value="external" {{ old('type', $meeting->type) == 'external' ? 'selected' : '' }}>خارجي</option>
                        <option value="online" {{ old('type', $meeting->type) == 'online' ? 'selected' : '' }}>أونلاين</option>
                        <option value="in-person" {{ old('type', $meeting->type) == 'in-person' ? 'selected' : '' }}>حضوري</option>
                        <option value="hybrid" {{ old('type', $meeting->type) == 'hybrid' ? 'selected' : '' }}>هجين</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="scheduled" {{ old('status', $meeting->status) == 'scheduled' ? 'selected' : '' }}>مجدول</option>
                        <option value="ongoing" {{ old('status', $meeting->status) == 'ongoing' ? 'selected' : '' }}>قيد التنفيذ</option>
                        <option value="completed" {{ old('status', $meeting->status) == 'completed' ? 'selected' : '' }}>مكتمل</option>
                        <option value="cancelled" {{ old('status', $meeting->status) == 'cancelled' ? 'selected' : '' }}>ملغي</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" name="start_time" id="start_time" 
                           value="{{ old('start_time', $meeting->start_time ? $meeting->start_time->format('Y-m-d\TH:i') : '') }}" required
                ...
                <div>
                    ...
                    <input type="datetime-local" name="end_time" id="end_time" 
                           value="{{ old('end_time', $meeting->end_time ? $meeting->end_time->format('Y-m-d\TH:i') : '') }}" required
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="participants[]" id="participants" multiple
                        ...
                        @foreach(\App\Models\User::whereHas('employee', function($q) { $q->where('status', 'active'); })->get() as $user)
                            <option value="{{ $user->id }}" {{ in_array($user->id, $selectedIds) ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
                <a href="{{ route('meetings.show', $meeting) }}" class="px-6 py-3 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
                    إلغاء
                </a>
