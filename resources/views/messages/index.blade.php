    ...
    <div class="mb-6">
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 sm:p-8 border border-blue-100">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <a href="{{ route('messages.create') }}" class="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl">
        ...
        <div class="flex flex-col sm:flex-row gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('messages.index', ['filter' => 'important']) }}" 
                   class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $filter === 'important' ? 'bg-yellow-100 text-yellow-700' : 'text-gray-600 hover:bg-gray-100' }}">
                    مهمة
                </a>
                <a href="{{ route('messages.index', ['filter' => 'sent']) }}" 
                   class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $filter === 'sent' ? 'bg-green-100 text-green-700' : 'text-gray-600 hover:bg-gray-100' }}">
                    مرسلة
                </a>
                <a href="{{ route('messages.index', ['filter' => 'urgent']) }}" 
                   class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $filter === 'urgent' ? 'bg-red-100 text-red-700' : 'text-gray-600 hover:bg-gray-100' }}">
                    عاجلة
                </a>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        @if($messages->count() > 0)
            <div class="divide-y divide-gray-200">
                ...
                <div class="p-4 sm:p-6 hover:bg-gray-50 transition-colors {{ !$message->is_read ? 'bg-blue-50' : '' }}">
                        ...
                        <div class="flex-shrink-0">
                            ...
                            @else
                                <div class="h-10 w-10 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center">
                                    <span class="text-sm font-bold text-white">{{ substr($message->sender->name, 0, 1) }}</span>
                                </div>
                            @endif
                        </div>
