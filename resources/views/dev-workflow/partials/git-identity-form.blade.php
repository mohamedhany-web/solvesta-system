...
<div class="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
    ...
    @if($identity)
    <span class="inline-flex text-xs font-bold px-2 py-1 rounded-lg mb-3 {{ $identity->statusColor() }}">{{ $identity->statusLabel() }}</span>
    @endif

    <form method="POST" action="{{ route('dev-workflow.git-identity') }}" class="space-y-3">
        @csrf
        <div>
            <label class="text-[10px] font-bold text-gray-600 block mb-1">GitHub Username</label>
            <input name="github_username" value="{{ old('github_username', $identity?->username) }}" required placeholder="username" dir="ltr"
        ...
        <div>
            ...
            <input type="email" name="github_email" value="{{ old('github_email', $identity?->email ?? auth()->user()?->email) }}" required dir="ltr"
                   class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm">
        </div>
        <div>
            <label class="text-[10px] font-bold text-gray-600 block mb-1">ملاحظة للإدارة (اختياري)</label>
            <input name="employee_note" value="{{ old('employee_note', $identity?->employee_note) }}" placeholder="مثال: مشروع Stock Management"
                   class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm">
        </div>
        <button type="submit" class="w-full py-2 rounded-xl text-white text-sm font-bold" style="background: {{ $themeColor }};">
            {{ $identity?->status === \App\Models\UserGitIdentity::STATUS_PENDING ? 'تحديث الطلب' : 'إرسال للإدارة' }}
        </button>
    </form>

    @if($identity?->status === \App\Models\UserGitIdentity::STATUS_APPROVED)
