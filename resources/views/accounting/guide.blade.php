...
@section('page-title', 'دليل نظام المحاسبة')

@section('content')
<div class="w-full max-w-4xl mx-auto prose prose-slate max-w-none">
    <div class="mb-8 not-prose">
        <a href="{{ route('accounting.index') }}" class="text-sm text-blue-600 font-semibold hover:underline">← لوحة التحكم المالية</a>
...
php artisan client-services:generate-invoices</pre>
    ...
    <div class="not-prose flex flex-wrap gap-3 mt-8">
        <a href="{{ route('accounting.client-services.index') }}" class="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-emerald-700">خدمات ما بعد البيع</a>
        <a href="{{ route('financial-invoices.index') }}" class="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-blue-700">الفواتير المالية</a>
        <a href="{{ route('accounting.wallets.index') }}" class="border px-5 py-2.5 rounded-xl font-bold hover:bg-gray-50">المحافظ</a>
    </div>
</div>
@endsection
