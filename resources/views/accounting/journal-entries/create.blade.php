...
@section('page-title', 'إنشاء قيد محاسبي جديد')
...
<div class="w-full max-w-6xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('accounting.journal-entries') }}" class="bg-gray-600 text-white px-6 py-3 rounded-xl hover:bg-gray-700 transition-all duration-200 flex items-center shadow-sm">
    ...
    <form action="{{ route('accounting.journal-entries.store') }}" method="POST" id="journalEntryForm">
        ...
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('accounting.journal-entries') }}" class="px-6 py-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors duration-200">
                إلغاء
            </a>
        ...
        <div class="mt-3">
            ...
            <div class="space-y-2 max-h-96 overflow-y-auto">
                ...
                <div class="border-b border-gray-200 pb-2 mb-2">
                    ...
                    <div class="flex items-center p-2 hover:bg-gray-50 cursor-pointer" onclick="selectAccount({{ $account->id }}, '{{ $account->name }}', '{{ $account->code }}')">
                        <span class="text-sm font-medium text-gray-700">{{ $account->code }} - {{ $account->name }}</span>
                    </div>
                    @endforeach
                </div>
                @endforeach
