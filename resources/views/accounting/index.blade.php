...
<div class="w-full">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl shadow-md border border-green-200 p-6 hover:shadow-lg hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-green-900 mb-1">{{ number_format($totalAssets) }}</p>
        ...
        <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl shadow-md border border-orange-200 p-6 hover:shadow-lg hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-orange-900 mb-1">{{ number_format($totalLiabilities) }}</p>
        ...
        <!-- Net Income -->
        <div class="bg-gradient-to-br {{ $netIncome >= 0 ? 'from-emerald-50 to-emerald-100 border-emerald-200' : 'from-red-50 to-red-100 border-red-200' }} rounded-xl shadow-md border p-6 hover:shadow-lg hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <div class="flex items-center mb-2">
                        <div class="p-2 {{ $netIncome >= 0 ? 'bg-emerald-500' : 'bg-red-500' }} rounded-lg ml-3">
                            <svg class="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $netIncome >= 0 ? 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6' : 'M13 17h8m0 0V9m0 8l-8-8-4 4-6-6' }}" />
                            </svg>
                        </div>
                        <p class="text-sm font-semibold {{ $netIncome >= 0 ? 'text-emerald-900' : 'text-red-900' }}">صافي الدخل</p>
                    </div>
                    <p class="text-2xl font-bold {{ $netIncome >= 0 ? 'text-emerald-900' : 'text-red-900' }} mb-1">{{ number_format(abs($netIncome)) }}</p>
                    <p class="text-xs font-medium {{ $netIncome >= 0 ? 'text-emerald-700' : 'text-red-700' }}">{{ $netIncome >= 0 ? 'ربح' : 'خسارة' }} - ج.م</p>
                </div>
            </div>
        </div>
                ...
                <div class="flex items-center justify-between p-5 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 hover:shadow-md transition-shadow duration-200">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-bold text-green-900">{{ number_format($monthlyRevenue) }} <span class="text-sm">ج.م</span></p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-xs text-green-600 font-medium">{{ $monthlyRevenue > 0 ? '+' : '' }}{{ number_format(($monthlyRevenue / max($totalRevenue, 1)) * 100, 1) }}%</p>
                ...
                <div class="flex items-center justify-between p-5 bg-gradient-to-r from-red-50 to-rose-50 rounded-xl border border-red-200 hover:shadow-md transition-shadow duration-200">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-bold text-red-900">{{ number_format($monthlyExpenses) }} <span class="text-sm">ج.م</span></p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-xs text-red-600 font-medium">{{ number_format(($monthlyExpenses / max($totalExpenses, 1)) * 100, 1) }}%</p>
                        <p class="text-xs text-gray-500">من الإجمالي</p>
                    </div>
                </div>
        ...
        <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-md border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-300">
            ...
            <div class="space-y-4">
                <div class="p-4 bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl border border-yellow-200 hover:shadow-md transition-shadow duration-200">
                    ...
                    <p class="text-2xl font-bold text-yellow-900 mr-11">{{ number_format($pendingInvoices) }} <span class="text-sm">ج.م</span></p>
                </div>
                <div class="p-4 bg-gradient-to-r from-orange-50 to-red-50 rounded-xl border border-orange-200 hover:shadow-md transition-shadow duration-200">
                    ...
                    <p class="text-2xl font-bold text-orange-900 mr-11">{{ number_format($pendingPayments) }} <span class="text-sm">ج.م</span></p>
                </div>
                <div class="p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl border border-purple-200">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-lg font-bold text-purple-900">{{ number_format($pendingInvoices + $pendingPayments) }} ج.م</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <!-- Chart of Accounts -->
        <a href="{{ route('accounting.accounts') }}" class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg border border-blue-700 p-6 hover:shadow-xl hover:scale-105 transition-all duration-300 group">
            <div class="flex items-center justify-between">
                <div class="flex-1">
        ...
        <!-- Journal Entries -->
        <a href="{{ route('accounting.journal-entries') }}" class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg border border-green-700 p-6 hover:shadow-xl hover:scale-105 transition-all duration-300 group">
            <div class="flex items-center justify-between">
                <div class="flex-1">
        ...
        <!-- Invoices -->
        <a href="{{ route('invoices.index') }}" class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg border border-purple-700 p-6 hover:shadow-xl hover:scale-105 transition-all duration-300 group">
            <div class="flex items-center justify-between">
                <div class="flex-1">
        ...
        <!-- Payments -->
        <a href="{{ route('payments.index') }}" class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg border border-red-700 p-6 hover:shadow-xl hover:scale-105 transition-all duration-300 group">
            <div class="flex items-center justify-between">
                <div class="flex-1">
    ...
    <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-md border border-gray-200 p-6 mb-8 hover:shadow-lg transition-shadow duration-300">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div class="text-center p-5 bg-gradient-to-br from-green-50 to-emerald-100 rounded-xl border border-green-200 hover:shadow-md transition-shadow duration-200">
                ...
                <p class="text-xl font-bold text-green-900">{{ number_format($totalAssets) }} <span class="text-sm">ج.م</span></p>
            </div>
            <div class="text-center p-5 bg-gradient-to-br from-orange-50 to-red-100 rounded-xl border border-orange-200 hover:shadow-md transition-shadow duration-200">
                ...
                <p class="text-xl font-bold text-orange-900">{{ number_format($totalLiabilities) }} <span class="text-sm">ج.م</span></p>
            </div>
            <div class="text-center p-5 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl border border-blue-200 hover:shadow-md transition-shadow duration-200">
                ...
                <p class="text-xl font-bold text-blue-900">{{ number_format($totalEquity ?? 0) }} <span class="text-sm">ج.م</span></p>
            </div>
            <div class="text-center p-5 bg-gradient-to-br from-purple-50 to-pink-100 rounded-xl border border-purple-200 hover:shadow-md transition-shadow duration-200">
                ...
                <p class="text-xl font-bold text-purple-900">{{ number_format($totalRevenue ?? 0) }} <span class="text-sm">ج.م</span></p>
            </div>
            <div class="text-center p-5 bg-gradient-to-br from-red-50 to-rose-100 rounded-xl border border-red-200 hover:shadow-md transition-shadow duration-200">
                ...
                <p class="text-xl font-bold text-red-900">{{ number_format($totalExpenses ?? 0) }} <span class="text-sm">ج.م</span></p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300">
        <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('accounting.journal-entries') }}" class="text-sm text-blue-600 hover:text-blue-700 font-semibold bg-blue-50 px-3 py-1 rounded-lg hover:bg-blue-100 transition-colors duration-200">عرض الكل</a>
            </div>
        </div>
                    ...
                    <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm font-medium text-gray-900">{{ $entry->date->format('Y-m-d') }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm font-semibold text-gray-900 bg-gray-100 px-2 py-1 rounded">{{ $entry->reference }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-600">{{ Str::limit($entry->description, 50) }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm font-bold text-gray-900">{{ number_format($entry->total_debit) }} <span class="text-xs text-gray-500">ج.م</span></span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {{ $entry->status_color }}">
                                {{ $entry->status_in_arabic }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('accounting.journal-entries.show', $entry) }}" class="text-blue-600 hover:text-blue-800 font-semibold bg-blue-50 px-3 py-1 rounded-lg hover:bg-blue-100 transition-colors duration-200">عرض</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <a href="{{ route('accounting.journal-entries.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors duration-200 text-sm font-medium">
                                    إنشاء قيد جديد
                                </a>
                            </div>
                        </td>
