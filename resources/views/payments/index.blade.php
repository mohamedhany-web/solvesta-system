    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex gap-3">
                <a href="{{ route('payments.create') }}" class="bg-blue-600 text-white px-6 py-3 rounded-xl hover:bg-blue-700 transition-all duration-200 flex items-center shadow-sm">
        ...
        <div class="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl shadow-md border border-blue-200 p-6 hover:shadow-lg transition-all duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-blue-900">{{ $totalPayments }}</p>
                    <p class="text-xs text-blue-600 mt-1">{{ $monthlyPayments }} هذا الشهر</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-green-50 to-emerald-100 rounded-xl shadow-md border border-green-200 p-6 hover:shadow-lg transition-all duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-900">{{ $incomingPayments }}</p>
                    <p class="text-xs text-green-600 mt-1">{{ number_format($totalIncoming) }} ج.م</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-red-50 to-rose-100 rounded-xl shadow-md border border-red-200 p-6 hover:shadow-lg transition-all duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-red-900">{{ $outgoingPayments }}</p>
                    <p class="text-xs text-red-600 mt-1">{{ number_format($totalOutgoing) }} ج.م</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-yellow-50 to-orange-100 rounded-xl shadow-md border border-yellow-200 p-6 hover:shadow-lg transition-all duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-yellow-900">{{ $pendingPayments }}</p>
                    <p class="text-xs text-yellow-600 mt-1">{{ number_format($pendingAmount) }} ج.م</p>
                </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-bold text-gray-900 bg-gray-100 px-2 py-1 rounded inline-block">{{ $payment->payment_number }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @endphp
                            <span class="text-sm font-bold {{ $isIncoming ? 'text-green-600' : 'text-red-600' }}">
                                {{ $isIncoming ? '+' : '-' }}{{ number_format($payment->amount) }} <span class="text-xs text-gray-500">ج.م</span>
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @php
                            ...
                            @if($payment->bankAccount)
                                <div class="text-xs text-gray-400 mt-1">{{ $payment->bankAccount->name }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                <div class="text-xs text-gray-600">
                                    <a href="{{ route('clients.show', $payment->client) }}" class="hover:text-blue-600">
                                        {{ $payment->client->name }}
                                    </a>
                                </div>
                                @elseif($payment->employee)
                                <div class="text-xs text-gray-600">{{ $payment->employee->name }}</div>
                                @endif
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('payments.show', $payment) }}" class="text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1 rounded-lg hover:bg-blue-100 transition-colors duration-200">عرض</a>
                                @if($payment->status === 'pending')
                                <button onclick="markAsCompleted({{ $payment->id }})" class="text-green-600 hover:text-green-800 bg-green-50 px-3 py-1 rounded-lg hover:bg-green-100 transition-colors duration-200">تأكيد</button>
                                @endif
                                @if($payment->status !== 'completed')
                                <button onclick="deletePayment({{ $payment->id }})" class="text-red-600 hover:text-red-800 bg-red-50 px-3 py-1 rounded-lg hover:bg-red-100 transition-colors duration-200">حذف</button>
                                @endif
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <a href="{{ route('payments.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors duration-200 text-sm font-medium">
                                    إضافة دفعة جديدة
                                </a>
                            </div>
                        </td>
