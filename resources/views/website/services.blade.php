...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
  <div class="text-center mb-10">
    <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-gray-200 bg-white text-sm font-bold text-gray-700 shadow-sm">
      <span class="h-2 w-2 rounded-full" style="background: {{ $themeColor }}"></span>
  ...
  {{-- منظومة كاملة: هندسة برمجيات + ذكاء اصطناعي وبيانات --}}
      ...
      @foreach($engCards as $c)
        <div class="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition text-right">
          <div class="h-12 w-12 rounded-xl flex items-center justify-center mb-4" style="background: color-mix(in srgb, {{ $themeColor }} 15%, transparent)">
            <svg class="h-6 w-6" style="color: {{ $themeColor }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $c['icon'] }}" />
            </svg>
          </div>
          <h4 class="text-lg font-extrabold text-gray-900">{{ $c['title'] }}</h4>
    ...
    <ol class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 text-right">
      @foreach([
        ...
        <li class="relative rounded-2xl border border-gray-200 bg-white p-5 pt-8 ps-14">
          <span class="absolute top-4 start-4 h-8 w-8 rounded-full flex items-center justify-center text-sm font-extrabold text-white" style="background: {{ $themeColor }}">{{ $step['n'] }}</span>
          <h3 class="font-extrabold text-gray-900">{{ $step['title'] }}</h3>
