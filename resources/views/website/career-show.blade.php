...
@section('title', \App\Helpers\SettingsHelper::getCompanyName() . ' — ' . $job->title)
...
<div class="sv-os" dir="ltr">
  <section class="sv-page-hero">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <a href="{{ route('website.careers') }}" class="inline-flex items-center gap-2 text-sm font-bold text-blue-600 hover:text-orange-500 transition mb-6">
      ...
      <div class="max-w-3xl">
        ...
        <h1 class="text-3xl sm:text-4xl font-black sv-display text-gray-900 leading-tight">{{ $job->title }}</h1>
        @if($job->summary)
        <p class="mt-4 text-lg text-gray-600 leading-relaxed">{{ $job->summary }}</p>
        @endif
      </div>
    </div>
  </section>
  ...
  <section class="sv-section max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    @if($errors->any() && empty($hideApplicationForm))
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
      <div class="lg:col-span-1 space-y-6">
        <div class="sv-contact-panel">
          ...
          <div class="prose prose-sm max-w-none text-gray-700 whitespace-pre-wrap leading-relaxed">{{ $job->description }}</div>
        </div>
        @if($job->requirements)
        <div class="sv-contact-panel">
          <h2 class="text-lg font-extrabold text-gray-900 mb-4">Requirements</h2>
          <div class="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{{ $job->requirements }}</div>
        </div>
        @endif
      </div>

      <div class="lg:col-span-2 sv-form-panel">
        @if($hideApplicationForm)
          <div class="text-center py-8 sm:py-12">
            ...
            <p class="mt-3 text-gray-600 text-sm leading-relaxed max-w-md mx-auto">
              Thank you for applying to <strong class="text-gray-900">{{ $job->title }}</strong>.
              Our team will review your application and contact you if there is a match.
            </p>
            @if(session('success'))
              <p class="mt-4 text-sm font-bold text-green-700">{{ session('success') }}</p>
            @endif
            <div class="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
              <a href="{{ route('website.careers') }}" class="sv-btn sv-btn-primary w-full sm:w-auto">Browse Other Roles</a>
              <a href="{{ route('website.contact') }}" class="sv-btn sv-btn-ghost w-full sm:w-auto">Contact Us</a>
            </div>
          </div>
        @else
          ...
          <form action="{{ route('website.careers.apply', $job->slug) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input name="linkedin_url" value="{{ old('linkedin_url') }}" class="sv-form-input" placeholder="https://linkedin.com/in/...">
              </div>
              <div class="md:col-span-2">
                <label class="block text-sm font-bold text-gray-700 mb-2">Portfolio / GitHub</label>
                <input name="portfolio_url" value="{{ old('portfolio_url') }}" class="sv-form-input" placeholder="https://...">
              </div>
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-2">Cover letter (optional)</label>
              <textarea name="message" rows="5" class="sv-form-textarea">{{ old('message') }}</textarea>
            </div>
            <div>
