...
@section('title', \App\Helpers\SettingsHelper::getCompanyName() . ' - الرئيسية')
...
<section class="relative overflow-hidden bg-gradient-to-b from-gray-50 to-white">
  <div class="absolute inset-0 pointer-events-none" aria-hidden="true">
    {{-- keep blobs inside viewport to avoid horizontal overflow on mobile --}}
    <div class="absolute -top-32 right-0 w-96 h-96 rounded-full opacity-20 blur-3xl translate-x-1/3" style="background:{{ $tc }}"></div>
    <div class="absolute bottom-0 left-0 w-80 h-80 rounded-full opacity-10 blur-3xl -translate-x-1/3" style="background:{{ $tc }}"></div>
  </div>
  ...
  <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-20 sm:pt-24 sm:pb-28">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
      ...
      <div class="pt-4">
        ...
        <div class="mt-6">
          <div class="text-xl sm:text-2xl font-extrabold font-cairo text-gray-900">{{ $cn }}</div>
          <h1 class="mt-3 text-3xl sm:text-4xl lg:text-5xl font-black font-cairo text-gray-900" style="line-height:1.2">
            <span class="block" style="color:{{ $tc }}">نبني حلولاً برمجية</span>
        ...
        <div class="mt-8 flex flex-col sm:flex-row gap-3">
          <a href="{{ route('website.contact') }}" class="inline-flex items-center justify-center px-7 py-3.5 rounded-2xl text-white font-extrabold btn-brand shadow-lg hover:shadow-xl transition-all">
            احجز جلسة استشارية
          </a>
          <a href="{{ route('website.services') }}" class="inline-flex items-center justify-center px-7 py-3.5 rounded-2xl border-2 border-gray-200 bg-white hover:bg-gray-50 font-bold text-gray-800 transition-all">
            استعرض خدماتنا
          </a>
        </div>
        ...
        <div class="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div class="bg-white rounded-xl border border-gray-200 p-3 text-center">
            <div class="text-xl font-black font-cairo" style="color:{{ $tc }}">+50</div>
            <div class="text-[11px] text-gray-500 mt-0.5">مشروع مُنجز</div>
          </div>
          <div class="bg-white rounded-xl border border-gray-200 p-3 text-center">
            <div class="text-xl font-black font-cairo" style="color:{{ $tc }}">B2B</div>
            <div class="text-[11px] text-gray-500 mt-0.5">حلول للشركات</div>
          </div>
          <div class="bg-white rounded-xl border border-gray-200 p-3 text-center">
            <div class="text-xl font-black font-cairo" style="color:{{ $tc }}">AI</div>
            <div class="text-[11px] text-gray-500 mt-0.5">ذكاء اصطناعي</div>
          </div>
          <div class="bg-white rounded-xl border border-gray-200 p-3 text-center">
            <div class="text-xl font-black font-cairo" style="color:{{ $tc }}">SLA</div>
      ...
      <div class="relative">
        {{-- Integrated micro-cards cluster (balanced grid + subtle overlap) --}}
        <div class="relative mx-auto max-w-xl">
          <div class="absolute inset-0 -z-10 pointer-events-none" aria-hidden="true">
            <div class="absolute -top-10 right-6 h-44 w-44 rounded-full blur-3xl opacity-20" style="background:{{ $tc }}"></div>
          </div>
          ...
          <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 sm:col-span-6 rounded-3xl border border-gray-200 bg-white shadow-xl p-6 min-h-[170px] flex flex-col">
              <div class="text-xs font-bold mb-1" style="color:{{ $tc }}">خدمات أساسية</div>
            ...
            <div class="col-span-12 sm:col-span-6 rounded-3xl border border-gray-200 bg-white shadow-xl p-6 min-h-[170px] flex flex-col">
              <div class="flex items-center justify-between">
                <div class="text-xs font-bold" style="color:{{ $tc }}">قيمة مضافة</div>
              ...
              <div class="mt-3 h-2 rounded-full bg-gray-100 overflow-hidden">
                <div class="h-full w-[78%] rounded-full" style="background:{{ $tc }}"></div>
              </div>
            ...
            <div class="col-span-12 rounded-3xl border border-gray-200 bg-white shadow-xl p-6 min-h-[138px]">
              <div class="flex items-center justify-between mb-3">
                <div class="text-xs font-bold" style="color:{{ $tc }}">بوابة العميل</div>
                <span class="text-[11px] px-2 py-0.5 rounded-full border border-gray-200 bg-white font-bold" style="color:{{ $tc }}">Secure</span>
              </div>
    ...
    <div class="flex flex-col items-center justify-center gap-4 sm:gap-6 text-center">
      ...
      <div class="grid grid-cols-4 gap-4 sm:gap-8 w-full sm:w-auto">
        <div class="text-center">
          <div class="text-xl sm:text-2xl font-black font-cairo" style="color:{{ $tc }}">+50</div>
          <div class="text-[11px] sm:text-xs text-gray-400">مشروع</div>
        </div>
        <div class="text-center">
          <div class="text-xl sm:text-2xl font-black font-cairo" style="color:{{ $tc }}">+30</div>
          <div class="text-[11px] sm:text-xs text-gray-400">عميل</div>
        </div>
        <div class="text-center">
          <div class="text-xl sm:text-2xl font-black font-cairo" style="color:{{ $tc }}">24/7</div>
          <div class="text-[11px] sm:text-xs text-gray-400">دعم</div>
        </div>
        <div class="text-center">
          <div class="text-xl sm:text-2xl font-black font-cairo" style="color:{{ $tc }}">99%</div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-7 hover:shadow-xl hover:border-gray-300 transition-all duration-300 group">
      <div class="h-12 w-12 rounded-xl flex items-center justify-center mb-5" style="background: color-mix(in srgb, {{ $tc }} 12%, transparent)">
        <svg class="h-6 w-6" style="color:{{ $tc }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<section class="bg-gray-50 border-y border-gray-100">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
    <div class="text-center mb-14">
      <h2 class="text-3xl sm:text-4xl font-black font-cairo text-gray-900">لماذا {{ $cn }}؟</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div class="bg-white rounded-2xl border border-gray-200 p-8 text-center hover:shadow-lg transition-all">
        <div class="mx-auto h-16 w-16 rounded-2xl flex items-center justify-center mb-6" style="background: color-mix(in srgb, {{ $tc }} 12%, transparent)">
          <svg class="h-8 w-8" style="color:{{ $tc }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <div class="relative bg-white rounded-2xl border border-gray-200 p-7 hover:shadow-lg transition-all">
      <div class="text-5xl font-black font-cairo opacity-10 absolute top-4 left-5" style="color:{{ $tc }}">{{ $s['n'] }}</div>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-5 text-center hover:shadow-lg transition-all group">
        <div class="mx-auto h-12 w-12 rounded-xl flex items-center justify-center mb-3" style="background: color-mix(in srgb, {{ $tc }} 10%, transparent)">
          <svg class="h-6 w-6 text-gray-500 group-hover:text-gray-900 transition-colors" style="color:{{ $tc }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $sec['icon'] }}" />
          </svg>
        </div>
        <div class="text-sm font-bold text-gray-900">{{ $sec['name'] }}</div>
      </div>
      @endforeach
    </div>
  </div>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
    <div>
      <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold border border-gray-200 bg-white mb-6" style="color:{{ $tc }}">
        بوابة العملاء
      </span>
      <h2 class="text-3xl sm:text-4xl font-black font-cairo text-gray-900" style="line-height:1.25">
        في Solvesta
        <span style="color:{{ $tc }}">العميل أولاً.</span>
      </h2>
      ...
      <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div class="rounded-2xl border border-gray-200 bg-white p-4">
          <div class="text-xs font-bold mb-1" style="color:{{ $tc }}">شفافية كاملة</div>
          <div class="text-sm font-extrabold text-gray-900">كل الخدمات والإنجازات في مكان واحد</div>
        </div>
        <div class="rounded-2xl border border-gray-200 bg-white p-4">
          <div class="text-xs font-bold mb-1" style="color:{{ $tc }}">دعم أسرع</div>
          <div class="text-sm font-extrabold text-gray-900">تذاكر أعطال + متابعة + تقارير</div>
        </div>
      </div>
      <a href="{{ route('client.login') }}" class="mt-8 inline-flex px-6 py-3 rounded-2xl text-white font-extrabold btn-brand shadow-lg hover:shadow-xl transition-all">
        جرّب بوابة العملاء
      </a>
    </div>
  ...
  <div class="rounded-3xl bg-gray-950 p-8 sm:p-14 text-white overflow-hidden relative">
    ...
    <div class="relative text-center max-w-2xl mx-auto">
      ...
      <div class="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
        <a href="{{ route('website.contact') }}" class="px-8 py-4 rounded-2xl text-white font-extrabold btn-brand shadow-lg hover:shadow-xl transition-all">
          احجز جلسة الآن
        </a>
        <a href="{{ route('client.login') }}" class="px-8 py-4 rounded-2xl border border-white/20 bg-white/10 hover:bg-white/15 font-bold transition-all">
          بوابة العملاء
        </a>
      </div>
    </div>
