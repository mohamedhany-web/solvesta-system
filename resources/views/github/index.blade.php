...
@section('page-title', 'تكامل GitHub')
...
<div class="w-full max-w-full font-tajawal">
    ...
    <div class="flex flex-wrap gap-2 mb-6 -mt-2">
        ...
        <a href="{{ route('dev-workflow.index') }}" class="px-4 py-2 rounded-xl bg-white border border-gray-200 text-sm font-bold text-gray-700 hover:bg-gray-50 shadow-sm">بيئة التطوير</a>
        @endcan
        <a href="{{ route('workspace.index') }}" class="px-4 py-2 rounded-xl bg-white border border-gray-200 text-sm font-bold text-gray-700 hover:bg-gray-50 shadow-sm">مساحة عملي</a>
        @if($connected && $activeAccount)
        <form method="POST" action="{{ route('github.refresh') }}">@csrf
            <input type="hidden" name="account" value="{{ $activeAccount->id }}">
            @if($selectedRepo)<input type="hidden" name="repo" value="{{ $selectedRepo }}">@endif
            <button type="submit" class="px-4 py-2 rounded-xl text-white text-sm font-bold shadow-sm" style="background: {{ $themeColor }};">تحديث الحساب النشط</button>
        </form>
        @endif
    </div>

    @if($connected && !empty($cacheMiss))
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div class="xl:col-span-4 space-y-5">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between gap-2">
                    ...
                    <span class="text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-lg">{{ $accounts->count() }}</span>
                </div>
                <div class="p-4 space-y-2 max-h-[42vh] overflow-y-auto">
                    @forelse($accounts as $acc)
                    @php $isActive = $activeAccount && $activeAccount->id === $acc->id; @endphp
                    <div class="rounded-xl border-2 p-3 transition {{ $isActive ? 'border-blue-400 bg-blue-50/40' : 'border-gray-100 hover:border-gray-200' }}">
                        <div class="flex items-start gap-3">
                            @if($acc->avatar_url)<img src="{{ $acc->avatar_url }}" alt="" class="w-10 h-10 rounded-full border border-white shadow shrink-0">@endif
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-1.5">
                                    <a href="{{ route('github.index', ['account' => $acc->id, 'tab' => $tab, 'repo' => $selectedRepo]) }}"
                                       class="font-bold text-sm text-gray-900 hover:underline truncate">{{ $acc->displayLabel() }}</a>
                                    @if($acc->is_default)<span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-amber-100 text-amber-800">افتراضي</span>@endif
                                    @if($isActive)<span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-green-100 text-green-800">نشط</span>@endif
                                </div>
                                <p class="text-[10px] text-gray-500 mt-0.5 font-mono" dir="ltr">@{{ $acc->login }}</p>
                                <p class="text-[10px] text-gray-400">{{ $acc->accountTypeLabel() }}@if(!$acc->isPersonal()) · {{ $acc->organization }}@endif</p>
                            </div>
                        </div>
                        @can('manage-github-integration')
                        <div class="flex flex-wrap gap-1.5 mt-2 pt-2 border-t border-gray-100/80">
                            @if(!$acc->is_default)
                            <form method="POST" action="{{ route('github.accounts.default', $acc) }}">@csrf
                                <button type="submit" class="text-[10px] font-bold px-2 py-1 rounded-lg border border-gray-200 hover:bg-white">افتراضي</button>
                            </form>
                            @endif
                            <form method="POST" action="{{ route('github.accounts.test', $acc) }}">@csrf
                                <button type="submit" class="text-[10px] font-bold px-2 py-1 rounded-lg border border-gray-200 hover:bg-white">اختبار</button>
                            </form>
                            <form method="POST" action="{{ route('github.accounts.destroy', $acc) }}" onsubmit="return confirm('حذف حساب {{ $acc->displayLabel() }}؟')">@csrf @method('DELETE')
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    ...
                    <form method="POST" action="{{ route('github.accounts.store') }}" class="space-y-3" id="github-connect-form">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">اسم تعريفي (اختياري)</label>
                            <input name="label" value="{{ old('label') }}" placeholder="مثال: حساب العميل أ / فريق الموبايل"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <label class="flex items-center gap-2 p-2.5 rounded-xl border-2 cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50 border-gray-200">
                                    <input type="radio" name="account_type" value="personal" class="text-blue-600" {{ old('account_type', 'personal') === 'personal' ? 'checked' : '' }} onchange="toggleOrgField()">
                                    <span class="text-xs font-bold">شخصي</span>
                                </label>
                                <label class="flex items-center gap-2 p-2.5 rounded-xl border-2 cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50 border-gray-200">
                                    <input type="radio" name="account_type" value="organization" class="text-blue-600" {{ old('account_type') === 'organization' ? 'checked' : '' }} onchange="toggleOrgField()">
                                    <span class="text-xs font-bold">Organization</span>
                                </label>
                            </div>
                        </div>
                        <div id="org-field" class="{{ old('account_type', 'personal') === 'organization' ? '' : 'hidden' }}">
                            <label class="block text-xs font-bold text-gray-600 mb-1">اسم Organization</label>
                            <input name="organization" value="{{ old('organization') }}" dir="ltr" placeholder="solvesta-org" class="w-full rounded-xl border-gray-300 text-sm font-mono">
                        </div>
                        <div>
                        ...
                        <button type="submit" class="w-full py-2.5 rounded-xl text-white font-bold text-sm" style="background: {{ $themeColor }};">+ إضافة حساب</button>
                    </form>
                </div>
            </div>
            @push('scripts')
            ...
            @if($connected && $rateLimit)
            <div class="bg-gray-900 rounded-2xl p-4 text-white text-sm">
                <p class="text-gray-400 text-xs mb-1">GitHub API Rate Limit</p>
                <p class="font-bold">{{ $rateLimit['remaining'] ?? '—' }} / {{ $rateLimit['limit'] ?? '—' }}</p>
                @if($syncedAt)<p class="text-[10px] text-gray-500 mt-2">آخر مزامنة: {{ \Carbon\Carbon::parse($syncedAt)->diffForHumans() }}</p>@endif
            </div>
            @endif

            @if($connected && $orgInfo)
            <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
                <h3 class="font-bold text-sm mb-2">{{ $isPersonal ? ($orgInfo['name'] ?? $orgInfo['login'] ?? 'حسابي') : ($orgInfo['name'] ?? $org) }}</h3>
                @if(!empty($orgInfo['bio']) || !empty($orgInfo['description']))
                <p class="text-xs text-gray-600">{{ $orgInfo['bio'] ?? $orgInfo['description'] }}</p>
                @endif
                <div class="grid grid-cols-2 gap-2 mt-3 text-center">
                    <div class="bg-gray-50 rounded-xl p-2"><p class="text-lg font-bold" style="color: {{ $themeColor }};">{{ count($repos) ?: ($stats['repos'] ?? '—') }}</p><p class="text-[10px] text-gray-500">مستودعات</p></div>
                    @if(!$isPersonal)
                    <div class="bg-gray-50 rounded-xl p-2"><p class="text-lg font-bold" style="color: {{ $themeColor }};">{{ count($teams) ?: ($stats['teams'] ?? '—') }}</p><p class="text-[10px] text-gray-500">فرق</p></div>
                    <div class="bg-gray-50 rounded-xl p-2"><p class="text-lg font-bold" style="color: {{ $themeColor }};">{{ count($members) ?: ($stats['members'] ?? '—') }}</p><p class="text-[10px] text-gray-500">أعضاء</p></div>
                    @else
                    <div class="bg-gray-50 rounded-xl p-2"><p class="text-lg font-bold" style="color: {{ $themeColor }};">{{ $orgInfo['public_repos'] ?? '—' }}</p><p class="text-[10px] text-gray-500">عامة على GitHub</p></div>
                    @endif
                    <div class="bg-gray-50 rounded-xl p-2"><p class="text-lg font-bold" style="color: {{ $themeColor }};">{{ count($pulls) ?: ($stats['pulls'] ?? '—') }}</p><p class="text-[10px] text-gray-500">PRs مفتوحة</p></div>
                </div>
            </div>
            @endif
        </div>
        ...
        <div class="xl:col-span-8">
            ...
            <div class="flex flex-wrap gap-1 mb-4 border-b border-gray-200 pb-0">
                ...
                <a href="{{ route('github.index', ['account' => $activeAccount?->id, 'tab' => $key, 'repo' => $selectedRepo]) }}"
                   class="px-4 py-2.5 text-sm font-bold rounded-t-xl border-b-2 -mb-px transition {{ $tab === $key ? 'border-current text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700' }}"
                   @if($tab === $key) style="border-color: {{ $themeColor }}; color: {{ $themeColor }};" @endif>{{ $label }}</a>
                @endforeach
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-gray-100 max-h-[70vh] overflow-y-auto" id="github-repos-list">
                    ...
                    @endphp
                    <div id="repo-row-{{ md5($fullName) }}"
                         class="px-5 py-4 hover:bg-gray-50/50 transition {{ $isHighlighted ? 'bg-blue-50/60 ring-2 ring-inset ring-blue-200' : '' }}"
                         data-repo-full-name="{{ $fullName }}"
                         data-default-branch="{{ $repo['default_branch'] ?? 'main' }}"
                         data-is-linked="{{ $isLinked ? '1' : '0' }}">
                        <div class="flex flex-wrap justify-between gap-3 items-start">
                            <div class="min-w-0 flex-1">
                                <a href="{{ $repo['html_url'] ?? '#' }}" target="_blank" rel="noopener" class="font-bold text-sm hover:underline" style="color: {{ $themeColor }};" dir="ltr">{{ $fullName }}</a>
                                @if(!empty($repo['description']))<p class="text-xs text-gray-500 mt-1 line-clamp-2">{{ $repo['description'] }}</p>@endif
                                <div class="flex flex-wrap gap-2 mt-2 text-[10px]">
                                    @if($repo['private'] ?? false)<span class="px-2 py-0.5 rounded bg-amber-100 text-amber-800 font-bold">Private</span>@else<span class="px-2 py-0.5 rounded bg-green-100 text-green-800">Public</span>@endif
                                    @if(!empty($repo['language']))<span class="px-2 py-0.5 rounded bg-blue-50 text-blue-700">{{ $repo['language'] }}</span>@endif
                                    <span class="text-gray-400">⭐ {{ $repo['stargazers_count'] ?? 0 }}</span>
                                    <span class="text-gray-400 font-mono">default: {{ $repo['default_branch'] ?? 'main' }}</span>
                                </div>
                                @if($isLinked)
                                <p class="text-[10px] text-green-600 mt-2 font-bold">✓ مربوط بمشروع: {{ $linkedRepos[strtolower($fullName)]->project?->name }}
                                    @if($linkedRepos[strtolower($fullName)]->githubAccount)
                                    <span class="text-gray-500 font-normal">· عبر {{ $linkedRepos[strtolower($fullName)]->githubAccount->displayLabel() }}</span>
                                    @endif
                                </p>
                                @endif
                            </div>
                            <div class="flex flex-col gap-2 shrink-0">
                                <button type="button" onclick="navigator.clipboard.writeText('git clone {{ $repo['clone_url'] ?? '' }}');this.textContent='تم!';"
                                        class="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200">نسخ clone</button>
                                <a href="{{ route('github.index', ['tab' => 'branches', 'repo' => $fullName]) }}" class="text-[10px] font-bold px-3 py-1.5 rounded-lg border border-gray-200 text-center hover:bg-gray-50">الفروع</a>
                                @can('manage-project-repos')
                                @if(!$isLinked)
                                <button type="button"
                                        onclick="openLinkRepoModal(@js($fullName), @js($repo['default_branch'] ?? 'main'))"
                                        class="text-[10px] font-bold px-3 py-1.5 rounded-lg text-white" style="background: {{ $themeColor }};">ربط بمشروع</button>
                                @endif
                                @endcan
                            </div>
                        </div>
                ...
                <div class="divide-y divide-gray-100 max-h-[70vh] overflow-y-auto">
                    ...
                    <div class="px-5 py-4 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-0.5">{{ $team['description'] ?? 'بدون وصف' }}</p>
                            <p class="text-[10px] text-gray-400 mt-1" dir="ltr">{{ $team['slug'] ?? '' }} · {{ $team['privacy'] ?? '' }}</p>
                        </div>
                        <a href="{{ $team['html_url'] ?? '#' }}" target="_blank" class="text-xs font-bold hover:underline" style="color: {{ $themeColor }};">GitHub</a>
                    </div>
                    @empty
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-0 divide-y sm:divide-y-0 sm:gap-3 sm:p-4 sm:divide-none max-h-[70vh] overflow-y-auto">
                    ...
                    <div class="px-5 py-3 sm:rounded-xl sm:border sm:border-gray-100 flex items-center gap-3">
                        ...
                        <div>
                            <a href="{{ $member['html_url'] ?? '#' }}" target="_blank" class="font-bold text-sm hover:underline" dir="ltr">{{ $member['login'] ?? '' }}</a>
                            <p class="text-[10px] text-gray-400">{{ $member['type'] ?? 'User' }}</p>
                        </div>
                    </div>
                    @empty
                ...
                <div class="divide-y divide-gray-100 max-h-[70vh] overflow-y-auto">
                    @forelse($pulls as $pr)
                    <div class="px-5 py-4">
                        <a href="{{ $pr['html_url'] ?? '#' }}" target="_blank" class="font-bold text-sm hover:underline line-clamp-2" style="color: {{ $themeColor }};">{{ $pr['title'] ?? '' }}</a>
                        <p class="text-xs text-gray-500 mt-1" dir="ltr">{{ $pr['repository_url'] ? basename(dirname($pr['repository_url'])).'/'.basename($pr['repository_url']) : '' }}</p>
                        <div class="flex gap-2 mt-2 text-[10px]">
                            <span class="px-2 py-0.5 rounded bg-green-100 text-green-800 font-bold">Open</span>
                            @if(!empty($pr['user']['login']))<span class="text-gray-500">by {{ $pr['user']['login'] }}</span>@endif
                            <span class="text-gray-400">{{ isset($pr['created_at']) ? \Carbon\Carbon::parse($pr['created_at'])->diffForHumans() : '' }}</span>
                        </div>
                    </div>
                    @empty
                ...
                <div class="p-4 border-b border-gray-100">
                    <form method="GET" action="{{ route('github.index') }}" class="flex flex-wrap gap-2 items-end">
                        <input type="hidden" name="tab" value="branches">
                        @if($activeAccount)<input type="hidden" name="account" value="{{ $activeAccount->id }}">@endif
                        <div class="flex-1 min-w-[200px]">
                            ...
                            <select name="repo" class="w-full rounded-xl border-gray-300 text-sm mt-1" onchange="this.form.submit()">
                                ...
                                @foreach($repos as $repo)
                                    <option value="{{ $repo['full_name'] }}" @selected($selectedRepo === ($repo['full_name'] ?? ''))>{{ $repo['full_name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </form>
                ...
                <div class="divide-y divide-gray-100 max-h-[60vh] overflow-y-auto">
                    ...
                    <div class="px-5 py-3 flex justify-between items-center">
                        ...
                        @if(!empty($branch['commit']['sha']))
                        <span class="text-[10px] font-mono text-gray-400" dir="ltr">{{ Str::limit($branch['commit']['sha'], 7, '') }}</span>
                        @endif
                    </div>
                    @empty
                ...
                <div class="p-4 border-b border-gray-100 flex flex-wrap gap-2 items-center justify-between">
                    ...
                    <form method="GET" action="{{ route('github.index') }}" class="flex gap-2">
                        ...
                        <select name="access_status" class="rounded-xl border-gray-300 text-xs" onchange="this.form.submit()">
                            ...
                            <option value="pending" @selected(request('access_status') === 'pending')>بانتظار الإدارة</option>
                            <option value="approved" @selected(request('access_status') === 'approved')>معتمد</option>
                            <option value="rejected" @selected(request('access_status') === 'rejected')>مرفوض</option>
                        </select>
                    </form>
                </div>
                <div class="divide-y divide-gray-100 max-h-[70vh] overflow-y-auto">
                        ...
                        <div class="flex flex-wrap justify-between gap-3 items-start">
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-xs text-gray-500">
                                    @if($deptName)<span>{{ $deptName }}</span><span class="text-gray-300 mx-1">·</span>@endif
                                    <a href="{{ $req->profile_url }}" target="_blank" rel="noopener" class="font-mono hover:underline" dir="ltr" style="color: {{ $themeColor }};">{{ $req->username }}</a>
                                    <span class="text-gray-300 mx-1">·</span>
                                    <span dir="ltr">{{ $req->email }}</span>
                                </p>
                                @if($req->employee_note)
                                <p class="text-xs text-gray-600 mt-1 bg-gray-50 rounded-lg px-2 py-1">ملاحظة الموظف: {{ $req->employee_note }}</p>
                                @endif
                                @if($req->admin_notes)
                                <p class="text-xs text-gray-500 mt-1">ملاحظة الإدارة: {{ $req->admin_notes }}</p>
                                @endif
                            ...
                            <div class="flex flex-wrap gap-2 shrink-0">
                                ...
                                <form method="POST" action="{{ route('github.access.approve', $req) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="px-3 py-1.5 rounded-xl text-white text-xs font-bold" style="background: {{ $themeColor }};">اعتماد وإرسال دعوات</button>
                                </form>
...
<div id="link-repo-modal" class="fixed inset-0 z-[100] hidden" aria-modal="true" role="dialog">
    ...
    <div class="absolute inset-x-4 top-[8vh] mx-auto max-w-lg bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden max-h-[84vh] flex flex-col">
        ...
        <form method="POST" action="{{ route('github.link-project') }}" id="link-repo-form" class="flex flex-col min-h-0 flex-1">
            @csrf
            ...
            @if($activeAccount)<input type="hidden" name="github_account_id" id="link-github-account-id" value="{{ $activeAccount->id }}">@endif
            <div class="p-5 space-y-4 overflow-y-auto flex-1">
                @if($activeAccount)
                <div class="p-3 rounded-xl bg-blue-50 border border-blue-100 text-xs text-blue-900">
                    سيتم الربط عبر حساب GitHub: <strong>{{ $activeAccount->displayLabel() }}</strong>
                    @if($accounts->count() > 1)
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-[45vh] overflow-y-auto pr-1" id="link-project-options">
                        ...
                        <label class="link-project-option flex items-start gap-3 p-3 rounded-xl border border-gray-200 hover:border-gray-300 hover:bg-gray-50/80 cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50/50 transition"
                               data-search="{{ strtolower($p->name.' '.($p->client?->name ?? '').' '.$p->status_name) }}">
                            <input type="radio" name="project_id" value="{{ $p->id }}" required
                                   class="mt-1 text-blue-600 shrink-0">
                            <div class="min-w-0 flex-1">
                                <p class="font-bold text-sm text-gray-900">{{ $p->name }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">
                                    @if($p->client)<span>{{ $p->client->name }}</span><span class="text-gray-300 mx-1">·</span>@endif
                    ...
                    <p class="text-[10px] text-gray-400 mt-2">{{ $linkableProjects->count() }} مشروع متاح للربط</p>
                    @endif
                </div>
            </div>
            <div class="px-5 py-4 border-t border-gray-100 bg-gray-50/50 flex flex-wrap gap-2 justify-end shrink-0">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-xl text-white text-sm font-bold" style="background: {{ $themeColor }};">ربط المستودع</button>
                @endif
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const autoRepo = @json($selectedRepo);
    const onReposTab = @json($tab === 'repos');
