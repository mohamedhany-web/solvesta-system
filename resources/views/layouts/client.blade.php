...
<div id="clientDrawer" class="fixed inset-0 z-50 hidden" style="max-width:100vw; overflow:hidden">
    ...
    <div id="clientPanel" class="absolute top-0 right-0 h-full w-[86%] max-w-sm bg-white shadow-2xl border-l border-gray-100"
        ...
        <div class="p-4 space-y-2 text-sm font-semibold text-gray-700">
            ...
            @endif
            <a href="{{ route('client.support.tickets') }}" class="block px-4 py-3 rounded-2xl border border-gray-200 bg-white hover:bg-gray-50 {{ request()->routeIs('client.support.*') ? 'ring-2 ring-black/5 text-gray-900' : '' }}">تذاكر الدعم</a>
            <a href="{{ route('client.help') }}" class="block px-4 py-3 rounded-2xl border border-gray-200 bg-white hover:bg-gray-50 {{ request()->routeIs('client.help') ? 'ring-2 ring-black/5 text-gray-900' : '' }}">شرح البورتال</a>
