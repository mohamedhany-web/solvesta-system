...
<head>
    ...
    <title>كود التحقق - {{ \App\Helpers\SettingsHelper::getSystemName() }}</title>
    <style>
        ...
        .email-header {
            background: linear-gradient(135deg, {{ \App\Helpers\SettingsHelper::getThemeColor() }} 0%, {{ \App\Helpers\SettingsHelper::getThemeColor() }}dd 100%);
            padding: 40px 30px;
            text-align: center;
            color: #ffffff;
        }
        ...
        .message-box {
            ...
            border-right: 4px solid {{ \App\Helpers\SettingsHelper::getThemeColor() }};
            padding: 20px;
            border-radius: 8px;
            margin: 20px 0;
            font-family: 'Tajawal', sans-serif;
        ...
        .company-name {
            color: {{ \App\Helpers\SettingsHelper::getThemeColor() }};
            font-weight: 600;
        }
                    ...
                    <table role="presentation" class="email-container" cellspacing="0" cellpadding="0" border="0" width="600">
                        ...
                        <tr>
                            <td class="email-header">
                                <div class="logo-container">
                                    ...
                                    @endphp
                                    @if($logoUrl)
                                        <img src="{{ $logoUrl }}" alt="Logo" style="width: 60px; height: 60px; object-fit: contain; display: block; margin: 0 auto; border: 0; outline: none; text-decoration: none;">
                                    @else
                                ...
                                <h1>{{ \App\Helpers\SettingsHelper::getSystemName() }}</h1>
                                <p>كود التحقق</p>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td class="email-content">
                                <div class="greeting">
                                    مرحباً <strong>{{ $userName ?? 'المستخدم' }}</strong>،
                                </div>
                                
                                <div class="message-box">
                                    <p>
                                        لقد قمت بتسجيل الدخول إلى نظام <strong>{{ \App\Helpers\SettingsHelper::getSystemName() }}</strong>. 
                                ...
                                <div class="code-container">
                                    ...
                                    <div class="code-display">{{ str_replace(' ', '', $code) }}</div>
                        ...
                        <tr>
                            <td class="email-footer">
                                ...
                                <p>
                                    هذه رسالة آلية من نظام <span class="company-name">{{ \App\Helpers\SettingsHelper::getSystemName() }}</span>
                                </p>
                                <p>
                                    يرجى عدم الرد على هذه الرسالة
                                </p>
                                <p style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e9ecef;">
                                    &copy; {{ date('Y') }} <span class="company-name">{{ \App\Helpers\SettingsHelper::getCompanyName() }}</span>. جميع الحقوق محفوظة.
                                </p>
                            </td>
                        </tr>
                    </table>
