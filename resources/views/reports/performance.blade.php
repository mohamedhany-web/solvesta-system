...
@section('page-title', 'تقرير الأداء')
...
<div class="w-full">
    <div class="mb-8">
        ...
        <p class="text-gray-600">من {{ $start_date }} إلى {{ $end_date }}</p>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($employeeMetrics as $metric)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $metric['name'] }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $metric['department'] }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            <div class="flex items-center">
                                <div class="w-20 bg-gray-200 rounded-full h-2 ml-2">
                                    <div class="bg-green-600 h-2 rounded-full" style="width: {{ $metric['attendance_rate'] }}%"></div>
                                </div>
                                <span class="font-medium">{{ $metric['attendance_rate'] }}%</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $metric['tasks_completed'] }}</td>
                        <td class="px-6 py-4">
