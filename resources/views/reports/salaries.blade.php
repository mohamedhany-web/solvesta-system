...
<div class="w-full">
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('reports.salaries.print', request()->query()) }}" target="_blank" class="bg-gray-800 text-white px-6 py-3 rounded-xl hover:bg-gray-900 transition-all duration-200 flex items-center shadow-lg hover:shadow-xl">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ number_format($summary['total_amount'], 0) }} ج.م</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <p class="text-sm font-medium text-gray-600 mb-1">الراتب الأساسي</p>
            <p class="text-2xl font-bold text-blue-600">{{ number_format($summary['total_basic'], 0) }} ج.م</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <p class="text-sm font-medium text-gray-600 mb-1">البدلات</p>
            <p class="text-2xl font-bold text-green-600">{{ number_format($summary['total_bonuses'], 0) }} ج.م</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <p class="text-sm font-medium text-gray-600 mb-1">الخصومات</p>
            <p class="text-2xl font-bold text-red-600">{{ number_format($summary['total_deductions'], 0) }} ج.م</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <p class="text-sm font-medium text-gray-600 mb-1">عدد السجلات</p>
            <p class="text-3xl font-bold text-gray-900">{{ $summary['total_salaries'] }}</p>
        </div>
    </div>
            ...
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $salary->employee->department->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm font-bold text-gray-900">{{ number_format($salary->base_salary, 0) }} ريال</td>
                        <td class="px-6 py-4">
                            @php
                                $totalAllowances = ($salary->bonus ?? 0) + ($salary->allowances ?? 0) + ($salary->overtime_amount ?? 0);
                            @endphp
                            <div class="text-sm font-medium text-green-600">{{ number_format($totalAllowances, 0) }} ريال</div>
                            @if($totalAllowances > 0)
                            <div class="text-xs text-gray-500">
                                @if($salary->bonus > 0)مكافأة: {{ number_format($salary->bonus, 0) }}<br>@endif
                                @if($salary->allowances > 0)بدلات: {{ number_format($salary->allowances, 0) }}<br>@endif
                                @if($salary->overtime_amount > 0)إضافي: {{ number_format($salary->overtime_amount, 0) }}@endif
                            </div>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            ...
                            @endphp
                            <div class="text-sm font-medium text-red-600">{{ number_format($totalDeductions, 0) }} ريال</div>
                            @if($totalDeductions > 0)
                            <div class="text-xs text-gray-500">
                                @if($salary->deductions > 0)خصومات: {{ number_format($salary->deductions, 0) }}<br>@endif
                                @if($salary->tax > 0)ضرائب: {{ number_format($salary->tax, 0) }}@endif
                            </div>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-sm font-bold text-blue-600">{{ number_format($salary->net_salary, 0) }} ريال</td>
