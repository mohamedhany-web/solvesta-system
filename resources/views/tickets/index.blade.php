...
<div class="w-full">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            @can('create-tickets')
            <a href="{{ route('tickets.create') }}" class="bg-blue-600 text-white px-6 py-3 rounded-xl hover:bg-blue-700 transition-all duration-200 flex items-center shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden w-full">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-900">قائمة التذاكر ({{ $tickets->total() }})</h3>
                <form method="GET" action="{{ route('tickets.index') }}" class="flex items-center gap-2">
                    <select name="status" onchange="this.form.submit()" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <option value="pending_client" {{ request('status') == 'pending_client' ? 'selected' : '' }}>في انتظار العميل</option>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="البحث عن تذكرة..." class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <a href="{{ route('tickets.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300">إلغاء</a>
                    @endif
                </form>
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="text-sm text-gray-500">{{ $ticket->category_name }}</div>
                        </td>
                        <td class="px-6 py-4">
                            @if($ticket->client)
                            <div class="flex items-center">
                                <div class="h-8 w-8 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center ml-2">
                                    <span class="text-xs font-medium text-white">{{ mb_substr($ticket->client->name, 0, 1) }}</span>
                                </div>
                                <div class="text-sm text-gray-900">{{ $ticket->client->name }}</div>
                            </div>
                            @else
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="flex items-center">
                                <div class="h-8 w-8 bg-gradient-to-r from-purple-500 to-purple-600 rounded-full flex items-center justify-center ml-2">
                                    <span class="text-xs font-medium text-white">{{ mb_substr($ticket->assignedTo->name, 0, 1) }}</span>
                                </div>
                        ...
                        <td class="px-6 py-4 text-sm font-medium">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('tickets.show', $ticket) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors duration-200">
                                ...
                                <a href="{{ route('tickets.edit', $ticket) }}" class="inline-flex items-center px-3 py-1.5 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition-colors duration-200">
                                    <svg class="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
