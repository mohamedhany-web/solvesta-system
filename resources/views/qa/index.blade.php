...
@section('page-title', 'الجودة والاختبارات')
...
<div class="w-full">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            @can('create-qa')
            <a href="{{ route('qa.create') }}" class="bg-blue-600 text-white px-6 py-3 rounded-xl hover:bg-blue-700 transition-all duration-200 flex items-center shadow-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $stats['passed'] }}</p>
                    <p class="text-xs text-green-600 mt-1">{{ $stats['pass_rate'] }}% معدل النجاح</p>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden w-full">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-900">قائمة الاختبارات ({{ $tests->total() }})</h3>
                <form method="GET" action="{{ route('qa.index') }}" class="flex items-center gap-2">
                    <select name="status" onchange="this.form.submit()" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <option value="passed" {{ request('status') == 'passed' ? 'selected' : '' }}>نجح</option>
                        <option value="failed" {{ request('status') == 'failed' ? 'selected' : '' }}>فشل</option>
                        <option value="skipped" {{ request('status') == 'skipped' ? 'selected' : '' }}>تم التخطي</option>
                    </select>
                    <select name="type" onchange="this.form.submit()" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <option value="unit" {{ request('type') == 'unit' ? 'selected' : '' }}>وحدة</option>
                        <option value="integration" {{ request('type') == 'integration' ? 'selected' : '' }}>تكامل</option>
                        <option value="functional" {{ request('type') == 'functional' ? 'selected' : '' }}>وظيفي</option>
                        <option value="performance" {{ request('type') == 'performance' ? 'selected' : '' }}>أداء</option>
                        <option value="security" {{ request('type') == 'security' ? 'selected' : '' }}>أمان</option>
                        <option value="usability" {{ request('type') == 'usability' ? 'selected' : '' }}>سهولة استخدام</option>
                    </select>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="البحث عن اختبار..." class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">بحث</button>
                    @if(request('search') || request('status') || request('type'))
                    <a href="{{ route('qa.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300">إلغاء</a>
                    @endif
                </form>
            </div>
        </div>
            ...
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900">{{ $test->test_number }}</div>
                            <div class="text-xs text-gray-500">{{ $test->created_at->diffForHumans() }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900">{{ Str::limit($test->name, 40) }}</div>
                            <div class="text-sm text-gray-500">{{ Str::limit($test->description, 50) }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900">{{ $test->project->name ?? 'عام' }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $typeColors[$test->type] ?? 'bg-gray-100 text-gray-800' }}">
                                {{ $test->type_name }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $statusColors[$test->status] ?? 'bg-gray-100 text-gray-800' }}">
                                {{ $test->status_name }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            ...
                            <div class="flex items-center">
                                <div class="h-8 w-8 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center ml-2">
                                    <span class="text-xs font-medium text-white">{{ mb_substr($test->assignedTo->name, 0, 1) }}</span>
                                </div>
                                <div class="text-sm text-gray-900">{{ $test->assignedTo->name }}</div>
                            </div>
                            @else
                        ...
                        <td class="px-6 py-4 text-sm font-medium">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('qa.show', $test) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors duration-200">
                                ...
                                <a href="{{ route('qa.edit', $test) }}" class="inline-flex items-center px-3 py-1.5 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition-colors duration-200">
                                    <svg class="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
