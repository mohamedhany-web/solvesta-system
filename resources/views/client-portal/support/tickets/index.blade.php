    ...
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-gray-600 text-sm sm:text-base">منظّمة حسب مشروعك — {{ $client->name }}</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('client.support.tickets.create') }}" class="bg-blue-600 text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 text-sm font-bold">تذكرة جديدة</a>
                <a href="{{ route('client.dashboard') }}" class="bg-gray-600 text-white px-5 py-2.5 rounded-lg hover:bg-gray-700 text-sm font-bold">العودة</a>
            </div>
        </div>
    </div>
    ...
    @forelse($projects as $project)
        <details class="mb-4 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden" open>
            <summary class="cursor-pointer list-none px-5 py-4 bg-slate-50 border-b flex flex-wrap items-center justify-between gap-2">
                ...
                <span class="text-sm font-semibold text-blue-700 bg-blue-50 px-3 py-1 rounded-full">{{ $project->tickets->count() }} تذكرة</span>
            </summary>
                ...
                <table class="w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">{{ $ticket->status_name }}</td>
                            <td class="px-4 py-3">{{ $ticket->priority }}</td>
                            <td class="px-4 py-3 text-left">
                                <a href="{{ route('client.support.tickets.show', $ticket) }}" class="text-blue-600 font-bold hover:underline">عرض</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    @if($unassignedTickets->isNotEmpty())
        <details class="mb-4 bg-white rounded-xl shadow-sm border border-amber-200 overflow-hidden" open>
            <summary class="cursor-pointer list-none px-5 py-4 bg-amber-50 border-b flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-amber-800">{{ $unassignedTickets->count() }}</span>
            </summary>
            <div class="overflow-x-auto">
                <table class="w-full divide-y divide-gray-200 text-sm">
                    <tbody class="divide-y divide-gray-100">
