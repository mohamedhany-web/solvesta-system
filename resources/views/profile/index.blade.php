...
<div class="rounded-2xl p-6 md:p-8 text-white mb-6 md:mb-8 shadow-xl relative overflow-hidden" style="background: linear-gradient(135deg, {{ $themeColor }} 0%, {{ $themeColor }}dd 100%); border: 2px solid {{ $themeColor }}30;">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between relative z-10 gap-4">
        ...
        <a href="{{ route('dashboard') }}" class="bg-white/20 backdrop-blur-sm text-white px-4 py-2 md:px-6 md:py-3 rounded-xl hover:bg-white/30 transition-all duration-200 flex items-center shadow-lg hover:shadow-xl transform hover:-translate-y-1 w-full md:w-auto justify-center gap-2 font-tajawal">
    ...
    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-6">
                    <div class="relative flex-shrink-0">
                        @if($user->profile_picture)
                            <img src="{{ asset('storage/' . $user->profile_picture) }}" alt="Profile Picture" class="h-24 w-24 sm:h-32 sm:w-32 md:h-36 md:w-36 rounded-full object-cover shadow-xl border-4 border-white" style="border-color: {{ $themeColor }}20;">
                        @else
                            <div class="h-24 w-24 sm:h-32 sm:w-32 md:h-36 md:w-36 rounded-full flex items-center justify-center shadow-xl border-4 border-white" style="background: linear-gradient(135deg, {{ $themeColor }} 0%, {{ $themeColor }}dd 100%); border-color: {{ $themeColor }}20;">
                                <span class="text-3xl sm:text-4xl md:text-5xl font-bold text-white font-tajawal">{{ substr($user->name, 0, 1) }}</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" required
                                   class="w-full pr-10 pl-4 py-3 md:py-4 border-2 border-gray-300 rounded-xl focus:ring-2 focus:border-transparent transition-all duration-200 text-gray-900 font-medium text-base md:text-lg @error('name') border-red-500 @enderror"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="password" id="current_password" name="current_password" 
                                   class="w-full pr-10 pl-4 py-3 md:py-4 border-2 border-gray-300 rounded-xl focus:ring-2 focus:border-transparent transition-all duration-200 text-gray-900 font-medium text-base md:text-lg @error('current_password') border-red-500 @enderror"
                                   placeholder="••••••••"
                                   style="focus:ring-color: {{ $themeColor }};">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="password" id="password" name="password" 
                                   class="w-full pr-10 pl-4 py-3 md:py-4 border-2 border-gray-300 rounded-xl focus:ring-2 focus:border-transparent transition-all duration-200 text-gray-900 font-medium text-base md:text-lg @error('password') border-red-500 @enderror"
                                   placeholder="••••••••"
                                   style="focus:ring-color: {{ $themeColor }};">
                        </div>
                        @error('password')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <p class="text-sm md:text-base text-gray-900 font-medium font-tajawal">{{ $user->created_at->format('Y/m/d') }}</p>
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <p class="text-sm md:text-base text-gray-900 font-medium font-tajawal">{{ $user->updated_at->format('Y/m/d H:i') }}</p>
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <p class="text-sm md:text-base text-gray-900 font-medium font-tajawal">{{ $user->roles->first()->name ?? 'لا يوجد دور' }}</p>
                            </div>
                        </div>
                        <div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-4 md:p-6" style="border-color: {{ $themeColor }}30;">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-3 sm:gap-4 pt-4 border-t border-gray-200">
                <a href="{{ route('dashboard') }}" class="px-6 py-3 text-sm md:text-base font-semibold text-gray-700 bg-white border-2 border-gray-300 rounded-xl hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-200 flex items-center justify-center gap-2 font-tajawal">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                    إلغاء
