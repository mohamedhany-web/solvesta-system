...
<div class="bg-gradient-to-r from-green-600 to-green-700 rounded-xl p-6 text-white mb-6 shadow-lg">
    <div class="flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-2xl font-bold mb-2">{{ $expense->expense_number }}</h1>
                <p class="text-green-100">{{ $expense->description }}</p>
            </div>
        </div>
        <div class="flex items-center space-x-3 space-x-reverse">
            <a href="{{ route('expenses.edit', $expense) }}" class="bg-white/20 backdrop-blur-sm text-white px-4 py-2 rounded-lg hover:bg-white/30 transition-all duration-200">
                تعديل
            </a>
            <a href="{{ route('expenses.index') }}" class="bg-white text-green-600 px-4 py-2 rounded-lg hover:bg-gray-100 transition-all duration-200">
                العودة
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="flex justify-between py-2 border-b">
                ...
                <span class="text-sm font-bold text-gray-900">{{ number_format($expense->amount, 2) }} ج.م</span>
            </div>
            <div class="flex justify-between py-2 border-b">
            ...
            <div class="flex justify-between py-2 border-b">
                ...
                <span class="text-sm text-gray-900">{{ $expense->expense_category }}</span>
            </div>
            <div class="flex justify-between py-2 border-b">
                <span class="text-sm font-medium text-gray-500">الحالة</span>
                <span class="text-sm text-gray-900">{{ $expense->status }}</span>
            </div>
            @if($expense->vendor)
            <div class="flex justify-between py-2 border-b">
                <span class="text-sm font-medium text-gray-500">المورد</span>
                <span class="text-sm text-gray-900">{{ $expense->vendor->name }}</span>
            </div>
            @endif
            <div class="flex justify-between py-2 border-b">
                <span class="text-sm font-medium text-gray-500">طريقة الدفع</span>
                <span class="text-sm text-gray-900">{{ $expense->payment_method }}</span>
            </div>
        </div>
    </div>
</div>
