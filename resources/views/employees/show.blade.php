    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center">
            <div class="h-16 w-16 bg-white/20 rounded-full flex items-center justify-center ml-4">
                <span class="text-2xl font-bold">{{ substr($employee->name, 0, 1) }}</span>
            </div>
            <div>
                <h1 class="text-2xl font-bold mb-2">{{ $employee->name }}</h1>
                <p class="text-green-100">{{ $employee->position }} - {{ $employee->department->name ?? 'غير محدد' }}</p>
            </div>
        </div>
        <div class="flex items-center space-x-3 space-x-reverse">
            <a href="{{ route('employees.edit', $employee) }}" class="bg-white/20 backdrop-blur-sm text-white px-4 py-2 rounded-lg hover:bg-white/30 transition-all duration-200">
                تعديل
            </a>
            <a href="{{ route('employees.index') }}" class="bg-white text-green-600 px-4 py-2 rounded-lg hover:bg-gray-100 transition-all duration-200">
                العودة
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex justify-between py-2 border-b border-gray-100">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-sm text-gray-900">{{ $employee->phone ?? 'غير محدد' }}</span>
                            @if($employee->phone)
                            <button onclick="openWhatsAppContact('{{ $employee->phone }}', '{{ addslashes($employee->name) }}')" 
                    ...
                    <div class="flex justify-between py-2 border-b border-gray-100">
                        ...
                        <span class="text-sm text-gray-900">{{ $employee->address ?? 'غير محدد' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="flex justify-between py-2 border-b border-gray-100">
                        ...
                        <span class="text-sm text-gray-900">{{ $employee->position ?? 'غير محدد' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-gray-100">
                        <span class="text-sm font-medium text-gray-500">القسم</span>
                        <span class="text-sm text-gray-900">{{ $employee->department->name ?? 'غير محدد' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-gray-100">
                        <span class="text-sm font-medium text-gray-500">الراتب</span>
                        <span class="text-sm text-gray-900">{{ $employee->salary ? number_format($employee->salary) . ' ج.م' : 'غير محدد' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-gray-100">
                        <span class="text-sm font-medium text-gray-500">تاريخ التوظيف</span>
                        <span class="text-sm text-gray-900">{{ $employee->hire_date ? \Carbon\Carbon::parse($employee->hire_date)->format('Y/m/d') : 'غير محدد' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-gray-100">
                        <span class="text-sm font-medium text-gray-500">الحالة</span>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                            @if($employee->status == 'active') bg-green-100 text-green-800
                            @elseif($employee->status == 'on_leave') bg-yellow-100 text-yellow-800
                            @else bg-red-100 text-red-800 @endif">
                            @if($employee->status == 'active') نشط
                            @elseif($employee->status == 'on_leave') في إجازة
                            @else غير نشط @endif
                        </span>
                    </div>
                </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <a href="{{ route('employees.edit', $employee) }}" class="w-full flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
                        <svg class="h-4 w-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                        تعديل البيانات
                    </a>
                    <a href="{{ route('attendances.index', ['employee' => $employee->id]) }}" class="w-full flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                        <svg class="h-4 w-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        سجل الحضور
                    </a>
                    <a href="{{ route('salaries.index', ['employee' => $employee->id]) }}" class="w-full flex items-center justify-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                        <svg class="h-4 w-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="space-y-3">
                    <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ $employee->created_at->diffForHumans() }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ $employee->updated_at->diffForHumans() }}</p>
                        </div>
                    </div>
                    @endif
                </div>
