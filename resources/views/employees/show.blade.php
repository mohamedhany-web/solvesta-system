...
<div class="w-full max-w-full font-tajawal">
    ...
    @include('employees.partials.kpi-panel')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
        <div class="xl:col-span-8 space-y-6">
                ...
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            ...
                            <dd class="font-semibold text-gray-900">{{ $fullName }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">البريد الإلكتروني</dt>
                            <dd class="font-semibold text-gray-900" dir="ltr">{{ $employee->email }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">رقم الهاتف</dt>
                            <dd class="flex items-center justify-between gap-2">
                                <span class="font-semibold text-gray-900" dir="ltr">{{ $employee->phone ?? '—' }}</span>
                                @if($employee->phone)
                                <button type="button" onclick="openWhatsAppContact('{{ $employee->phone }}', @js($fullName))"
                        ...
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4 sm:col-span-2">
                            ...
                            <dd class="text-gray-800">{{ $employee->address ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
                ...
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            ...
                            <dd class="font-mono font-semibold text-gray-900">{{ $employee->employee_id }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">الحالة</dt>
                            <dd><span class="text-xs font-bold px-2.5 py-1 rounded-full {{ $statusClass }}">{{ $statusLabel }}</span></dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">المنصب</dt>
                            <dd class="font-semibold text-gray-900">{{ $employee->position ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">القسم</dt>
                            <dd class="font-semibold text-gray-900">{{ $employee->department->name ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">الراتب الشهري</dt>
                            <dd class="font-semibold text-gray-900">{{ $employee->salary ? number_format($employee->salary).' ج.م' : '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">ساعات العمل اليومية</dt>
                            <dd class="text-gray-800">{{ $employee->daily_hours ?? '—' }} ساعة</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">تاريخ التوظيف</dt>
                            <dd class="text-gray-800">{{ $employee->hire_date?->format('Y/m/d') ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">نوع التوظيف</dt>
                            <dd class="text-gray-800">{{ $employmentLabel }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
                ...
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            ...
                            <dd class="text-gray-800">{{ $employee->emergency_contact ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">الهاتف</dt>
                            <dd class="text-gray-800" dir="ltr">{{ $employee->emergency_phone ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between gap-3">
                    ...
                    <a href="{{ route('users.show', $employee->user) }}" class="text-xs font-bold hover:underline" style="color: {{ $themeColor }};">عرض المستخدم →</a>
                </div>
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                            <dt class="text-xs font-bold text-gray-500 mb-1">اسم المستخدم</dt>
                            <dd class="font-semibold text-gray-900">{{ $employee->user->name }}</dd>
                        </div>
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                        ...
                        <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4 sm:col-span-2">
                            ...
                            <dd class="flex flex-wrap gap-2">
                                ...
                                <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-blue-100 text-blue-800">{{ $role->name }}</span>
                                @endforeach
                            </dd>
                        </div>
                        @endif
        ...
        <div class="xl:col-span-4 space-y-4">
            <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm text-center">
                ...
                <p class="text-sm text-gray-600 mt-1">{{ $employee->department->name ?? '—' }}</p>
                <div class="mt-4">
                    <span class="text-xs font-bold px-3 py-1 rounded-full {{ $statusClass }}">{{ $statusLabel }}</span>
                </div>
                <p class="text-xs text-gray-400 mt-3 font-mono" dir="ltr">{{ $employee->employee_id }}</p>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                ...
                <dl class="space-y-3 text-sm">
                    <div class="flex justify-between items-center py-2 border-b border-gray-100">
                        ...
                        <dd class="font-bold text-gray-900">{{ $stats['total_attendance_days'] ?? 0 }}</dd>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-100">
                        <dt class="text-gray-500">طلبات الإجازة</dt>
                        <dd class="font-bold text-gray-900">{{ $stats['total_leaves'] ?? 0 }}</dd>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-100">
                        <dt class="text-gray-500">إجازات معلّقة</dt>
                        <dd class="font-bold text-amber-700">{{ $stats['pending_leaves'] ?? 0 }}</dd>
                    </div>
                    <div class="flex justify-between items-center py-2">
                        <dt class="text-gray-500">إجازات معتمدة</dt>
                        <dd class="font-bold text-emerald-700">{{ $stats['approved_leaves'] ?? 0 }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-2">
                ...
                @if($employee->user)
                <a href="{{ route('kpi.show', $employee->user) }}"
            ...
            <div class="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-5 text-sm">
                ...
                <ul class="space-y-2 text-indigo-800">
                    <li>أُنشئ {{ $employee->created_at->diffForHumans() }}</li>
                    @if($employee->updated_at->ne($employee->created_at))
                    <li>آخر تحديث {{ $employee->updated_at->diffForHumans() }}</li>
                    @endif
                </ul>
            </div>
        </div>
