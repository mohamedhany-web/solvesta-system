...
<div class="w-full">
    ...
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            ...
            <a href="{{ route('employees.create') }}" class="bg-blue-600 text-white px-5 py-2.5 sm:px-6 sm:py-3 rounded-lg hover:bg-blue-700 transition-all duration-200 flex items-center justify-center shadow-sm text-sm sm:text-base">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $employees->total() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $employees->where('status', 'active')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $employees->where('status', 'on_leave')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">{{ number_format($employees->avg('salary') ?? 0) }} ج.م</p>
        ...
        <div class="px-4 sm:px-5 lg:px-6 py-3 sm:py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                ...
                <form method="GET" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="البحث..." class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="p-4 hover:bg-gray-50">
                <div class="flex items-start gap-3 mb-3">
                    <div class="h-12 w-12 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
                        <span class="text-base font-medium text-white">{{ substr($employee->first_name, 0, 1) }}</span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-semibold text-gray-900 truncate">{{ $employee->first_name }} {{ $employee->last_name }}</div>
                        <div class="text-xs text-gray-500 truncate">{{ $employee->email }}</div>
                        <div class="text-xs text-gray-600 mt-1">{{ $employee->position }}</div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-2 text-xs mb-3">
                    <div>
                        ...
                        <span class="font-medium text-gray-900">{{ $employee->department->name ?? 'غير محدد' }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">الراتب:</span>
                        <span class="font-medium text-gray-900">{{ number_format($employee->salary) }} ج.م</span>
                    </div>
                </div>
                <a href="{{ route('employees.show', $employee) }}" class="block w-full text-center px-3 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors duration-200 text-sm font-medium">
                    عرض التفاصيل
                </a>
            </div>
            @empty
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="h-10 w-10 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center ml-3 flex-shrink-0">
                                    <span class="text-sm font-medium text-white">{{ substr($employee->first_name, 0, 1) }}</span>
                                </div>
                                <div class="min-w-0">
                                    <div class="text-sm font-medium text-gray-900 truncate">{{ $employee->first_name }} {{ $employee->last_name }}</div>
                                    <div class="text-sm text-gray-500 truncate">{{ $employee->email }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                            {{ number_format($employee->salary) }} ج.م
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                @endif
                                <a href="{{ route('employees.show', $employee) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors duration-200 text-xs font-medium">
