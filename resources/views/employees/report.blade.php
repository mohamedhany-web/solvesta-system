...
@section('page-title', 'تقرير الموظف')
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('employees.index') }}" class="bg-gray-600 text-white px-6 py-3 rounded-xl hover:bg-gray-700 transition-all duration-200 flex items-center shadow-sm">
                    <svg class="h-5 w-5 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-6 space-x-reverse">
                <div class="h-20 w-20 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center shadow-lg">
                    <span class="text-2xl font-bold text-white">{{ substr($employee->first_name, 0, 1) }}</span>
                </div>
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">{{ $employee->first_name }} {{ $employee->last_name }}</h2>
                    <p class="text-gray-600">{{ $employee->position }}</p>
                    <p class="text-sm text-gray-500">{{ $employee->department->name ?? 'غير محدد' }}</p>
                </div>
            </div>
            <div class="text-left">
                <div class="text-sm text-gray-500">تاريخ التوظيف</div>
                <div class="text-lg font-semibold text-gray-900">{{ $employee->hire_date->format('Y/m/d') }}</div>
                <div class="text-sm text-gray-500">الراتب الشهري: ${{ number_format($employee->salary) }}</div>
                <div class="text-sm text-gray-500">الساعات اليومية: {{ $employee->daily_hours ?? 8 }} ساعة</div>
            </div>
        </div>
    </div>
