...
@section('page-title', 'تفاصيل البرنامج التدريبي')
...
<div class="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 text-white mb-6 shadow-lg">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3 space-x-reverse">
            ...
            @endcan
            <a href="{{ route('training.index') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100 transition-all duration-200">
                العودة
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-900 whitespace-pre-wrap">{{ $training->description ?? 'لا يوجد وصف' }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            ...
                            <span class="text-sm text-gray-900">{{ $training->start_date->format('Y-m-d') }}</span>
                        </div>

                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="text-sm font-medium text-gray-500">تاريخ الانتهاء</span>
                            <span class="text-sm text-gray-900">{{ $training->end_date->format('Y-m-d') }}</span>
                        </div>

                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="text-sm font-medium text-gray-500">الحد الأقصى للمشاركين</span>
                            <span class="text-sm text-gray-900">{{ $training->max_participants }}</span>
                        </div>

                        @if($training->cost > 0)
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="text-sm font-medium text-gray-500">التكلفة</span>
                            <span class="text-sm text-gray-900">{{ number_format($training->cost, 2) }} ج.م</span>
                        </div>
                        @endif
                        ...
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            ...
                            <span class="text-sm text-gray-900">{{ $training->instructor->name }}</span>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-sm text-gray-500">{{ $training->participants->count() }} / {{ $training->max_participants }}</span>
                </div>

                @can('edit-training')
                <!-- Add Participant Form -->
                <form action="{{ route('training.add-participant', $training) }}" method="POST" class="mb-6 pb-6 border-b border-gray-200">
                    @csrf
                ...
                @if($training->participants->count() > 0)
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            ...
                            @can('edit-training')
                            <form action="{{ route('training.remove-participant', [$training, $participant]) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{ $training->start_date->diffInDays($training->end_date) + 1 }} يوم</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">المشاركون المسجلون</span>
                        <span class="text-sm font-medium text-gray-900">{{ $training->participants->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">المقاعد المتاحة</span>
                        <span class="text-sm font-medium text-gray-900">{{ $training->max_participants - $training->participants->count() }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <form action="{{ route('training.destroy', $training) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا البرنامج التدريبي؟');">
                    @csrf
                    @method('DELETE')
