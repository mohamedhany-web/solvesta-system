...
@section('page-title', 'تعديل البرنامج التدريبي')
...
<div class="w-full max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('training.show', $training) }}" class="bg-gray-600 text-white px-6 py-3 rounded-xl hover:bg-gray-700 transition-all duration-200 flex items-center shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <form action="{{ route('training.update', $training) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" required
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $training->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="internal" {{ old('type', $training->type) == 'internal' ? 'selected' : '' }}>داخلي</option>
                        <option value="external" {{ old('type', $training->type) == 'external' ? 'selected' : '' }}>خارجي</option>
                        <option value="online" {{ old('type', $training->type) == 'online' ? 'selected' : '' }}>أونلاين</option>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="ongoing" {{ old('status', $training->status) == 'ongoing' ? 'selected' : '' }}>قيد التنفيذ</option>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $training->start_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $training->end_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="max_participants" id="max_participants" value="{{ old('max_participants', $training->max_participants) }}" required min="1"
                ...
                <div>
                    ...
                    <input type="number" name="cost" id="cost" value="{{ old('cost', $training->cost) }}" min="0" step="0.01"
                ...
                <div>
                    ...
                    <select name="department_id" id="department_id" required
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ old('department_id', $training->department_id) == $department->id ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
                <a href="{{ route('training.show', $training) }}" class="px-6 py-3 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
                    إلغاء
                </a>
