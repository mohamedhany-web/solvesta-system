<?php $__env->startSection('page-title', 'إدارة صلاحيات المستخدم'); ?>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3 sm:gap-4">
                    <div class="h-12 w-12 sm:h-14 sm:w-14 lg:h-16 lg:w-16 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0">
                        <span class="text-lg sm:text-xl lg:text-2xl font-bold text-white"><?php echo e(mb_substr($user->name, 0, 1)); ?></span>
                    </div>
                    <div class="min-w-0">
                ...
                <a href="<?php echo e(route('roles.index')); ?>" class="bg-white text-indigo-600 px-5 py-2.5 rounded-lg hover:bg-gray-50 transition-all duration-200 shadow-sm text-sm text-center">
                    العودة
                </a>
            </div>
        </div>
        ...
        <div class="p-4 sm:p-6">
            <form action="<?php echo e(route('roles.assign-role', $user)); ?>" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
                    ...
                    ?>
                    <label class="relative flex items-start p-3 sm:p-4 border-2 rounded-lg cursor-pointer hover:border-<?php echo e($roleInfo['color']); ?>-400 transition-all <?php echo e($isSelected ? 'border-' . $roleInfo['color'] . '-500 bg-' . $roleInfo['color'] . '-50' : 'border-gray-200'); ?>">
                        <input type="radio" name="role" value="<?php echo e($role->name); ?>" class="mt-1 ml-2 sm:ml-3 flex-shrink-0" <?php echo e($isSelected ? 'checked' : ''); ?> onchange="this.form.submit()">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center">
                                <div class="h-7 w-7 sm:h-8 sm:w-8 bg-<?php echo e($roleInfo['color']); ?>-600 rounded-lg flex items-center justify-center ml-2 flex-shrink-0">
                                    <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                    </svg>
                                </div>
                                <p class="text-xs sm:text-sm font-semibold text-gray-900"><?php echo e($roleInfo['name']); ?></p>
                            </div>
                            <p class="text-xs text-gray-500 mt-1 mr-9 sm:mr-10"><?php echo e($roleInfo['desc']); ?></p>
                            <p class="text-xs text-<?php echo e($roleInfo['color']); ?>-600 mt-1 mr-9 sm:mr-10"><?php echo e($role->permissions->count()); ?> صلاحية</p>
                        </div>
                    </label>
        ...
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 px-8 py-6 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600">الصلاحيات الممنوحة تلقائياً من دور: <span class="font-semibold text-green-700"><?php echo e(\App\Helpers\RoleHelper::getRoleName($userRole->name)); ?></span></p>
                </div>
                <span class="bg-green-100 text-green-800 px-4 py-2 rounded-lg text-sm font-semibold">
                    <?php echo e($userRole->permissions->count()); ?> صلاحية نشطة
                </span>
            </div>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div class="flex items-center p-3 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <p class="text-sm font-medium text-green-900"><?php echo e($permission->name); ?></p>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
            ...
            <form action="<?php echo e(route('roles.assign-permissions', $user)); ?>" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6" id="permissionsGrid">
                    ...
                    <label class="flex items-start p-4 rounded-lg cursor-pointer transition-all permission-item
                        <?php echo e($isChecked ? ($isFromRole && !$hasCustomOverride ? 'bg-green-50 border-2 border-green-300 hover:bg-green-100' : 'bg-purple-50 border-2 border-purple-300 hover:bg-purple-100') : ($isExplicitlyDisabled ? 'bg-red-50 border-2 border-red-300 hover:bg-red-100' : 'bg-gray-50 border-2 border-gray-200 hover:bg-gray-100')); ?>">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center justify-between">
                                ...
                                <?php elseif($hasCustomOverride && $customPermissionsMap[$permission->name]): ?>
                                <span class="text-xs bg-purple-200 text-purple-800 px-2 py-1 rounded-full">✓ مضافة</span>
                                <?php elseif($isExplicitlyDisabled): ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\solvesta\resources\views\roles\user-permissions.blade.php ENDPATH**/ ?>