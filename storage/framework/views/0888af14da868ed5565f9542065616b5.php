...
<div class="space-y-4">
    <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm text-center">
        ...
        <p class="text-lg font-bold text-gray-900"><?php echo e(now()->locale('ar')->translatedFormat('l')); ?></p>
        <p class="text-sm text-gray-600 mt-1" dir="ltr"><?php echo e(now()->format('Y-m-d')); ?></p>
    </div>

    <div class="grid grid-cols-2 gap-3">
        <div class="rounded-2xl border border-gray-200 bg-white p-4 text-center shadow-sm">
            <p class="text-2xl font-bold" style="color: <?php echo e($themeColor); ?>;"><?php echo e($stats['week_hours'] ?? 0); ?></p>
            <p class="text-xs text-gray-500 mt-1">ساعات الأسبوع</p>
        </div>
        <div class="rounded-2xl border border-gray-200 bg-white p-4 text-center shadow-sm">
            <p class="text-2xl font-bold <?php echo e(($stats['open_blockers'] ?? 0) > 0 ? 'text-red-600' : 'text-emerald-600'); ?>"><?php echo e($stats['open_blockers'] ?? 0); ?></p>
    ...
    <a href="<?php echo e(route('workspace.index')); ?>" class="block rounded-2xl border border-gray-200 bg-white p-4 text-sm font-semibold text-gray-700 hover:bg-gray-50 shadow-sm text-center">
        ← العودة لمساحة العمل
    </a>
</div>
<?php /**PATH C:\xampp\htdocs\solvesta\resources\views\daily-reports\partials\form-sidebar.blade.php ENDPATH**/ ?>