<?php $__env->startSection('page-title', 'إدارة المبيعات'); ?>
...
<div class="w-full">
    ...
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            ...
            <a href="<?php echo e(route('sales.create')); ?>" class="bg-blue-600 text-white px-5 py-2.5 sm:px-6 sm:py-3 rounded-lg hover:bg-blue-700 transition-all duration-200 flex items-center justify-center shadow-sm text-sm sm:text-base">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900"><?php echo e($sales->total()); ?></p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900"><?php echo e($sales->where('stage', 'closed_won')->count()); ?></p>
                    <p class="text-xs text-green-600 mt-1"><?php echo e($sales->total() > 0 ? round(($sales->where('stage', 'closed_won')->count() / $sales->total()) * 100, 1) : 0); ?>% من إجمالي المبيعات</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900"><?php echo e($sales->whereNotIn('stage', ['closed_won', 'closed_lost'])->count()); ?></p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900"><?php echo e(number_format($sales->where('stage', 'closed_won')->sum('actual_value') ?? $sales->where('stage', 'closed_won')->sum('estimated_value') ?? 0)); ?> ج.م</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        <form method="GET" action="<?php echo e(route('sales.index')); ?>" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- Search -->
            <div>
            ...
            <div>
                ...
                <select name="stage" id="stage" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <?php $__currentLoopData = $stages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stage): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($stage); ?>" <?php echo e(request('stage') == $stage ? 'selected' : ''); ?>>
                            <?php if($stage == 'lead'): ?> عميل محتمل
                            <?php elseif($stage == 'prospect'): ?> عميل مؤهل
                            <?php elseif($stage == 'proposal'): ?> عرض سعر
                            <?php elseif($stage == 'negotiation'): ?> مفاوضات
                            <?php elseif($stage == 'closed_won'): ?> مكتمل - فائز
                            <?php elseif($stage == 'closed_lost'): ?> مكتمل - خاسر
                            <?php else: ?> <?php echo e($stage); ?>
            ...
            <div class="md:col-span-4 flex items-end gap-2">
                ...
                <a href="<?php echo e(route('sales.index')); ?>" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors duration-200">
                    مسح
                </a>
            </div>
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 w-1/6">
                            <div>
                                <div class="text-sm font-medium text-gray-900"><?php echo e($sale->product_service); ?></div>
                                <div class="text-sm text-gray-500"><?php echo e($sale->lead_source ?? 'غير محدد'); ?></div>
                            </div>
                        </td>
                        <td class="px-6 py-4 w-1/6">
                            <div class="flex items-center">
                                <div class="h-8 w-8 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center ml-3">
                                    <span class="text-xs font-medium text-white"><?php echo e(substr($sale->client->name, 0, 1)); ?></span>
                                </div>
                                <div>
                                    <div class="text-sm font-medium text-gray-900"><?php echo e($sale->client->name); ?></div>
                                    <div class="text-sm text-gray-500"><?php echo e($sale->client->company_name ?? 'غير محدد'); ?></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 w-1/6 text-sm text-gray-900">
                            <?php echo e($sale->salesRep->name); ?>

                        </td>
                        <td class="px-6 py-4 w-1/6">
                            <div>
                                <div class="text-sm font-medium text-gray-900"><?php echo e(number_format($sale->actual_value ?? $sale->estimated_value)); ?> ج.م</div>
                                <div class="text-sm text-gray-500"><?php echo e($sale->probability_percentage); ?>% احتمال</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 w-1/6">
                        ...
                        <td class="px-6 py-4 w-1/6">
                            <div class="flex items-center gap-2">
                                <a href="<?php echo e(route('sales.show', $sale)); ?>" class="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors duration-200">
                                ...
                                <a href="<?php echo e(route('sales.edit', $sale)); ?>" class="inline-flex items-center px-3 py-1.5 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition-colors duration-200">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <?php if($sale->stage == 'closed_won'): ?>
                                    <?php if(isset($invoices[$sale->id])): ?>
                                        <a href="<?php echo e(route('invoices.show', $invoices[$sale->id])); ?>" class="inline-flex items-center px-3 py-1.5 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition-colors duration-200" title="عرض الفاتورة الموجودة">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <?php else: ?>
                                        <form action="<?php echo e(route('sales.generate-invoice', $sale)); ?>" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من إنشاء الفاتورة؟');">
                                            <?php echo csrf_field(); ?>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <a href="<?php echo e(route('sales.create')); ?>" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\solvesta\resources\views\sales\index.blade.php ENDPATH**/ ?>